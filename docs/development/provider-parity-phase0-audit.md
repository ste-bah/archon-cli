...
| `src/session/build_agent_provider.rs` | 1 | Build the active Agent's backing client during session setup (extracted from `src/session/build_agent.rs`). | Replace remaining low-level construction with provider router where practical. |
...
| `src/runtime/llm.rs` | 1 | Runtime provider wrapper (one site wrapped in ObservedLlmProvider by PRD-006). Since bdbea37ce it is spelled `AnthropicClient::with_read_backstop`, the same construction with the transport backstop sized from `[subagent] stream_idle_timeout_secs`; the guard counts both spellings. | Approved low-level factory; command/pipeline/gametheory callers should use it instead of direct construction. |
