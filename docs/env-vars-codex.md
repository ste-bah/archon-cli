...
| `ARCHON_CODEX_APP_SERVER_URL` | URL | config value | PRD-006 | Overrides `[providers.openai-codex].app_server_url` for local app-server diagnostics. Must be `http` or `https`; invalid values fail closed. The adapter is fail-closed until implemented. |
