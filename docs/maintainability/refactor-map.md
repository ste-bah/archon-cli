...
- Do not raise a gate's threshold to clear a red. Both TUI duplication gates stay pinned at 5% and the duplication was extracted instead — see [the decision record](../decisions/rejected/2026-08-22-raise-the-tui-duplication-threshold.md), which also explains why the two gates report different numbers over the same directory, and [`docs/defensive-patterns.md`](../defensive-patterns.md) DP-16.
...
| 5 | Command surface and CLI registry | `src/command/registry.rs`, `src/cli_args.rs` | 84 primary commands, aliases, help output, slash mirror |
