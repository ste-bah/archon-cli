...
- Credentials: `~/.archon/.credentials.json`
...
- Unsupported surfaces are visible in the capability matrix.
...
| Codex OAuth | Login with `archon auth login --provider openai-codex --accept-tos` | Doctor reports Codex OAuth present; `archon chat --provider openai-codex "Say ARCHON_SMOKE_OK_42"` works. |
