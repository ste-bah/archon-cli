...
| `/workflow` | — | Dynamic workflow control: `plan <task>`, `run <task>`, `status <run-id>`, `resume <run-id>`, `restart-agent <run-id> <stage-id>`, `save <run-id> <name>`, `list`. TUI `plan`/`run`/`resume` use the active provider in-process, show Agent Activity rows for execution, and store durable state under `.archon/workflows/<run-id>`. `list` and `status` open the workflow inspection view. |
