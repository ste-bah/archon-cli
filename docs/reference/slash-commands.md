...
As of v1.3.10 the registry contains **83 primary commands** (lockstep-tested at `EXPECTED_COMMAND_COUNT = 83` in `src/command/registry.rs` and `EXPECTED_PRIMARY_COUNT = 83` in `src/command/dispatcher.rs`). Aliases come from each handler's `aliases()` method.
