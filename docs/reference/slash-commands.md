...
As of v1.3.9 the registry contains **82 primary commands** (lockstep-tested at `EXPECTED_COMMAND_COUNT = 82` in `src/command/registry.rs` and `EXPECTED_PRIMARY_COUNT = 82` in `src/command/dispatcher.rs`). Aliases come from each handler's `aliases()` method.
