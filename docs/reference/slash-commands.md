...
As of v1.2.0 the registry contains **80 primary commands** (lockstep-tested at `EXPECTED_COMMAND_COUNT = 80` in `src/command/registry.rs` and `EXPECTED_PRIMARY_COUNT = 80` in `src/command/dispatcher.rs`). Aliases come from each handler's `aliases()` method.
