...
As of v1.3.11 the registry contains **84 primary commands** (lockstep-tested at `EXPECTED_COMMAND_COUNT = 84` in `src/command/registry.rs` and `EXPECTED_PRIMARY_COUNT = 84` in `src/command/dispatcher.rs`). Aliases come from each handler's `aliases()` method.
