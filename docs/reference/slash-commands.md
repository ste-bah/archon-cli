...
As of v1.3.11 the registry contains **83 primary commands** (lockstep-tested at `EXPECTED_COMMAND_COUNT = 83` in `src/command/registry.rs` and `EXPECTED_PRIMARY_COUNT = 83` in `src/command/dispatcher.rs`). Aliases come from each handler's `aliases()` method.
...
| `/workflow` | — | Dynamic workflow control: `plan <task>`, `run <task>`, `run --spec-file <path>`, `run --from-template <name>`, `status <run-id>`, `resume <run-id>`, `restart-agent <run-id> <stage-id>`, `force-accept <run-id> <stage-id> <rationale>`, `save <run-id> <name>`, `list`. TUI `plan`/`run`/`resume` use the active provider in-process, show Agent Activity rows for execution, and store durable state under `.archon/workflows/<run-id>`. `list` and `status` open the workflow inspection view. |
