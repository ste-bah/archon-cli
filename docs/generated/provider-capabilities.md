...
| `openai-codex` | ChatGPT/Codex OAuth | yes | yes | yes | yes | yes | yes | yes | yes | yes | yes | no | no | Backs one-shot chat, full TUI sessions, tool use, subagents, /btw, and provider-neutral pipelines; exact cost metadata remains unavailable when the backend omits usage pricing. |
