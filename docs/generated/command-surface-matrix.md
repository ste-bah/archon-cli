...
| `archon world ...` | - | World-model shell | SHELL_ONLY | `src/cli_args.rs + src/command/world_model.rs + crates/archon-world-model` | Local world-model status, ingest/backfill, dynamic trainer tick, candidate train/eval/promote, fail-open prediction, outcome/surprise recording, action scoring, explain, and rollback are shell-first while the advisor remains advisory-only. | World-model controls are intentionally shell-first until a dedicated TUI inspector and approval workflow lands.; owner: archon-maintainers; review: 2026-09-30 |
