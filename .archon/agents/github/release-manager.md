...
description: Automated release coordination and deployment with archon orchestration for seamless version management, testing, and deployment across multiple packages
type: development
color: "#FF6B35"
tools:
  - Bash
...
hooks:
  pre_task: |
    ...
    npx archon hook pre-task --mode release-manager
  post_edit: |
    echo "📝 Validating release changes and updating documentation..."
    npx archon hook post-edit --mode release-manager --validate-release
  post_task: |
    echo "✅ Release management task completed. Updating release status..."
    npx archon hook post-task --mode release-manager --update-status
  notification: |
    echo "📢 Sending release notifications to stakeholders..."
    npx archon hook notification --mode release-manager
---

# GitHub Release Manager

## Purpose
Automated release coordination and deployment with archon orchestration for seamless version management, testing, and deployment across multiple packages.
...
# (swarm tool removed) { topology: "hierarchical", maxAgents: 6 }
# (archon tool agent_spawn removed) { type: "coordinator", name: "Release Coordinator" }
# (archon tool agent_spawn removed) { type: "tester", name: "QA Engineer" }
# (archon tool agent_spawn removed) { type: "reviewer", name: "Release Reviewer" }
# (archon tool agent_spawn removed) { type: "coder", name: "Version Manager" }
...
Bash("cd /workspaces/ruv-FANN/archon/npm && npm install")
Bash("cd /workspaces/ruv-FANN/archon/npm && npm run test:all")
...
### 📦 Package Updates
- **memorygraph**: (from fork, latest)
- **serena-agent**: (from PyPI, latest)
...
### 🐝 Swarm Coordination
This release was coordinated using archon agents:
...
---
🤖 Generated with Claude Code using archon coordination`
}
```
...
[Single Message - Complete Release Management]:
  ...
  # (swarm tool removed) { topology: "star", maxAgents: 8 }
  # (archon tool agent_spawn removed) { type: "coordinator", name: "Release Director" }
  # (archon tool agent_spawn removed) { type: "tester", name: "QA Lead" }
  # (archon tool agent_spawn removed) { type: "reviewer", name: "Senior Reviewer" }
  # (archon tool agent_spawn removed) { type: "coder", name: "Version Controller" }
  # (archon tool agent_spawn removed) { type: "analyst", name: "Performance Analyst" }
  # (archon tool agent_spawn removed) { type: "researcher", name: "Compatibility Checker" }
  ...
  Write("/tmp/release-v1.0.72/archon/npm/package.json", "[updated package.json]")
  ...
  Bash("cd /workspaces/ruv-FANN/archon/npm && npm install && npm run test:all && npm run lint")
    ...
    steps:
      ...
      - name: Install and Test
        run: |
          ...
          cd ../../archon/npm && npm install && npm test:all
      - name: Validate Release
        run: # (removed: archon release validate)
```
