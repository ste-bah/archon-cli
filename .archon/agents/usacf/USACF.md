...
```bash
# (init handled by MemoryGraph MCP)
...
# Export metrics
# (removed: claude-flow hooks session-end --export-metrics > metrics.json)
...
```bash
# 1. Initialize
# (init handled by MemoryGraph MCP) && # (memory managed by MemoryGraph MCP)
