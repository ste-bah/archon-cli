...
# (removed: claude-flow health-check --components ["swarm", "agents", "coordination"])

# Collect specific metrics
# (removed: claude-flow metrics-collect --components ["cpu", "memory", "network"])

# Monitor SLA compliance
# (removed: claude-flow sla-monitor --service swarm-coordination --threshold 99.9)
```

### Alert Configuration
```bash
# Configure performance alerts
# (removed: claude-flow alert-config --metric cpu_usage --threshold 80 --severity warning)

# Set up anomaly detection
# (removed: claude-flow anomaly-setup --models ["statistical", "ml", "time_series"])

# Configure notification channels
# (removed: claude-flow notification-config --channels ["slack", "email", "webhook"])
```
