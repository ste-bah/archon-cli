...
fn smoke_fork_with_active_session_returns_branchpoint_and_increments_count() {
    ...
    let store =
        Arc::new(SessionStore::open(&temp_dir.path().join("fork_test.db")).expect("open store"));
    ...
    assert!(
        result.is_ok(),
        "fork with active session should succeed, got: {:?}",
        result
    );
    ...
    assert_eq!(
        bp.parent_session, parent_id,
        "BranchPoint.parent_session should match"
    );
    assert_eq!(
        bp.branched_at_message, 3,
        "BranchPoint.branched_at_message should be 3"
    );
...
fn smoke_fork_without_active_session_returns_err() {
    ...
    assert!(
        result.is_err(),
        "fork without active session should return Err, got: {:?}",
        result
    );
