use archon_tui::screens::settings_screen::{SettingsScreen, SettingField};
...
fn test_text_field_shows_key_value() {
    let mut screen = SettingsScreen::new();
    screen.set_fields(vec![
        SettingField::Text { key: "username".into(), value: "alice".into() },
    ]);
...
fn test_enum_field_options() {
    let mut screen = SettingsScreen::new();
    screen.set_fields(vec![
        SettingField::Enum { key: "mode".into(), value: "a".into(), options: vec!["a".into(), "b".into(), "c".into()] },
    ]);
