...
pub struct IngestResult {
    ...
    /// Integrity tally across PDFs: how many landed in `COORD_MARKER` / `COORD_PDF_NATIVE`
    /// (real bboxes) vs `COORD_NONE` (bbox-less text fallback). For the corpus re-ingest,
    /// `pdf_coord_none == 0` is the at-a-glance proof that no document silently degraded.
    pub pdf_coord_marker: usize,
    pub pdf_coord_native: usize,
...
fn add_pdf_counts(result: &mut IngestResult, file: &IngestFileResult) {
    ...
    match file.pdf_coord {
        ...
        Some(crate::block_chunking::COORD_PDF_NATIVE) => result.pdf_coord_native += 1,
