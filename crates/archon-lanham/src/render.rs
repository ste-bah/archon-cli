...
mod tests {
    ...
    /// tests/profile_sample.json + tests/ref_sample.md are produced by the Bash harness.
    #[test]
    fn render_matches_mjs_reference() {
        let dir = env!("CARGO_MANIFEST_DIR");
        let profile_json = std::fs::read_to_string(format!("{dir}/tests/profile_sample.json"))
            .expect("profile_sample.json missing — run the render-golden harness");
        let reference = std::fs::read_to_string(format!("{dir}/tests/ref_sample.md"))
            .expect("ref_sample.md missing — run the render-golden harness");
        let profile: Profile = serde_json::from_str(&profile_json).unwrap();
        let rendered = render_output_style(&profile, "reference-scholarly-sample0001");
        assert_eq!(
            rendered, reference,
