...
//!
//! The analyzer is split by axis into submodules; this file holds the shared
//! tokenization/foundation primitives, the orchestration (`full_analysis`,
//! `train_to_output_style`), and the flat public re-exports.
...
pub use lexical::{
    get_content_words, is_be_verb, is_latinate, is_nominalization, is_verb,
    latinate_germanic_ratio, rough_stem,
};
pub use noun_verb::{NounVerbMetrics, analyze_noun_verb};
pub use parataxis::{ParataxisMetrics, analyze_parataxis};
pub use prosody::{
    PeriodicMetrics, RegisterMetrics, VoiceMetrics, analyze_periodic, analyze_register,
    analyze_voice,
};
pub use tacit::{TacitPatterns, detect_tacit};

use opacity::analyze_opacity;

// ── Foundation: tokenization + shared primitives ─────────────────────────────
...
pub(crate) static AND_RE: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\band\b").unwrap());
...
pub(crate) fn count_markers(markers: &[Regex], text: &str) -> usize {
