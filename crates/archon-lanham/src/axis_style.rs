//! Periodic/running, voice and register axes.
//!
...
use crate::axis_syntax::*;
use crate::lexicon::*;

// ── Data + regexes for the remaining axes ────────────────────────────────────
pub(crate) const FORMAL_MARKERS_ARR: &[&str] = &[
    "furthermore",
    "moreover",
    "nevertheless",
    "notwithstanding",
...
pub(crate) static FORMAL_MARKERS: Lazy<HashSet<&'static str>> =
    Lazy::new(|| FORMAL_MARKERS_ARR.iter().copied().collect());

pub(crate) static CONTRACTION_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\b[A-Za-z0-9_]+'[A-Za-z0-9_]+\b").unwrap());
pub(crate) static PASSIVE_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\b(?:is|are|was|were|been|be)\s+[A-Za-z0-9_]+ed\b").unwrap());
pub(crate) static FILLER_RE: Lazy<Regex> = Lazy::new(|| {
...
pub(crate) static IMPERSONAL_RE: Lazy<Regex> =
...
pub(crate) static PARTICIPLE_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^[A-Z][a-z]+(?:ing|ed)\b").unwrap());

pub(crate) static PERSONALITY_MARKERS: Lazy<Vec<Regex>> = Lazy::new(|| {
    [
