...
fn audit_limits_survive_real_configuration_loading() {
    ...
    assert_eq!(
        config["workflow"]["repository_audit"]["attempt_timeout_secs"],
        7200
    );
    assert_eq!(
        config["workflow"]["repository_audit"]["total_time_secs"],
        "unlimited"
    );
    assert_eq!(
        config["workflow"]["repository_audit"]["unexpected_change_refreshes"],
        12
    );
}
...
fn malformed_audit_layer_is_not_skipped() {
    ...
    assert!(
        ...
        "invalid audit policy was silently skipped"
    );
}
...
fn invalid_layered_audit_value_names_its_source_file_and_key() {
    ...
    assert!(
        ...
        "invalid policy lost source attribution: {error}"
    );
    assert!(
        error.contains("total_time_secs"),
        "invalid policy lost key attribution: {error}"
    );
}
...
fn layered_policy_retains_effective_source_and_inheritance() {
    ...
    .unwrap();
    let config =
        archon_core::config_layers::load_layered_config(Some(&user), &project, None, None).unwrap();
    ...
    .unwrap();
    assert_eq!(
        resolved["sources"]["attempt_timeout_secs"]["path"],
        user.to_str().unwrap()
    );
    assert_eq!(
        resolved["sources"]["total_time_secs"]["path"],
        local.to_str().unwrap()
    );
    assert_eq!(
        resolved["sources"]["attempt_timeout_secs"]["key"],
        "workflow.generated.host_call_timeout_secs"
    );
}
