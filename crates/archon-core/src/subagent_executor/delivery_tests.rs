...
fn fixture(message_to_lead: bool) -> (tempfile::TempDir, AgentSubagentExecutor, Arc<Provider>) {
    ...
    let executor = AgentSubagentExecutor::new(
        provider.clone(),
        crate::dispatch::create_default_registry(temp.path().into(), None),
...
async fn delivery_executor_installs_discovery_on_actual_tool_registry() {
    ...
    let result = catalog
        .execute(
            json!({"action":"info","name":"reviewer"}),
            &ToolContext::default(),
        )
        .await;
...
async fn delivery_unknown_specialist_fails_before_provider_call() {
    ...
    assert!(
        result.is_err(),
        "unknown specialist must not run generically"
    );
...
async fn delivery_actual_runner_reports_full_result_and_progress_to_parent() {
    ...
    assert!(
        messages.iter().any(|m| m == "child progress"),
        "{messages:?}"
    );
    assert!(
        messages.iter().any(|m| m.contains(&report())),
        "full result must reach parent: {messages:?}"
    );
    assert!(
        manager
    ...
    assert!(
        serde_json::to_string(&requests[0].system)
            .unwrap()
            .contains("SPECIALIST_PROMPT")
    );
    assert!(
        !serde_json::to_string(requests[0].tools.as_ref())
            .unwrap()
            .contains("\"name\":\"Agent\"")
    );
}
...
async fn delivery_result_lookup_uses_retained_name_without_resuming() {
    ...
    let mut manager = executor.subagent_manager.lock().await;
    manager
        .register_with_id("finished-child".into(), request("reviewer"))
        .unwrap();
    ...
    let context = crate::message_router::RouterContext::new(
        executor.subagent_manager.clone(),
        crate::message_router::SenderIdentity::Lead,
    );
    let host = NoResume;
    let result =
        crate::message_router::maybe_route_send_message(&context, &host, "SendMessage", envelope)
            .await;
    assert!(!result.is_error, "{}", result.content);
    assert_eq!(
...
async fn delivery_result_lookup_reports_running_and_failed_without_restart() {
    ...
    executor
        ...
        .lock()
        .await
        .register_with_id("pending".into(), request("reviewer"))
        .unwrap();
    let context = crate::message_router::RouterContext::new(
        executor.subagent_manager.clone(),
        crate::message_router::SenderIdentity::Lead,
    ...
    for status in ["running", "failed"] {
        if status == "failed" {
            executor
                ...
                .lock()
                .await
                .mark_failed("pending", "failure evidence".into())
                .unwrap();
        }
        let envelope = archon_tools::send_message::SendMessageTool
            .execute(
                json!({"to":"pending", "message_type":"result"}),
                &ToolContext::default(),
            )
            .await;
        let result = crate::message_router::maybe_route_send_message(
            &context,
            &NoResume,
            "SendMessage",
            envelope,
        ...
        if status == "failed" {
            assert_eq!(data["error"], "failure evidence");
        }
    }
    assert!(provider.requests.lock().unwrap().is_empty());
}
