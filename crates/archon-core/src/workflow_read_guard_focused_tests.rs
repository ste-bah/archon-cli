...
fn completion_message_arrives_once_after_every_declared_test_exits_zero() {
    ...
    guard.after_tool("Bash", &bash(DECLARED[0]), false);
    ...
    guard.after_tool("Bash", &bash("cd crates/x &&  cargo   nextest run -p x --test a"), true);
    ...
    guard.after_tool("Bash", &bash(DECLARED[1]), true);
...
fn past_the_grace_allowance_inspection_and_tests_are_refused_but_writes_admitted() {
    ...
    for command in DECLARED {
        ...
        guard.after_tool("Bash", &bash(command), true);
    }
...
fn a_session_with_no_declared_tests_never_receives_the_message() {
    ...
    plain.after_tool("Bash", &bash(DECLARED[0]), true);
