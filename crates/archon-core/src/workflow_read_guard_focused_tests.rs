...
fn completion_message_arrives_once_after_every_declared_test_exits_zero() {
    ...
    assert!(
        guard
            .before_tool(
                "Bash",
                &bash("cd crates/x &&  cargo   nextest run -p x --test a")
            )
            .is_none()
    );
    guard.after_tool(
    ...
    assert!(
        guard.completion_message().is_none(),
        "one of two is not completion"
    );
    assert!(guard.before_tool("Bash", &bash(DECLARED[1])).is_none());
    guard.after_tool("Bash", &bash(DECLARED[1]), true, "exit 0");
    let message = guard
        .completion_message()
        .expect("completion after both passed");
    assert!(
        message.contains(
            "All declared focused tests have passed in this session (2 of 2 at tool call 3)"
        ),
        "{message}"
    );
...
fn past_the_grace_allowance_inspection_and_tests_are_refused_but_writes_admitted() {
    ...
    assert!(
        guard
            .before_tool("Read", &json!({"file_path": "a.rs"}))
            .is_none()
    );
    assert!(
        guard
            .before_tool("Bash", &bash("cargo check -p x"))
            .is_none()
    );
    assert!(
        guard
            .before_tool("Grep", &json!({"pattern": "fn"}))
            .is_none()
    );
    // The fourth inspection is refused with the instruction repeated.
    let refused = guard
        .before_tool("Bash", &bash("cat src/lib.rs"))
        .expect("inspection refused");
    assert!(
        refused.contains("All declared focused tests have passed in this session (2 of 2"),
        "{refused}"
    );
    ...
    assert!(
        guard
            .before_tool("Bash", &bash("cargo test -p x"))
            .is_some(),
        "a test run is refused"
    );
    assert!(
        guard
            .before_tool("Read", &json!({"file_path": "a.rs"}))
    ...
    assert!(
        guard
            .before_tool("Write", &json!({"file_path": "a.rs", "content": "x"}))
            .is_none()
    );
    assert!(
        guard
            .before_tool("Edit", &json!({"file_path": "a.rs"}))
            .is_none()
    );
}
