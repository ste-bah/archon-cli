    ...
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        impl<'de> de::Visitor<'de> for Visitor {
            ...
            fn expecting(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
                f.write_str(
                    "a positive integer representable in milliseconds or the string \"unlimited\"",
                )
            }
            fn visit_u64<E: de::Error>(self, n: u64) -> Result<Self::Value, E> {
                ...
                if n == 0 || n > (i64::MAX as u64) / 1000 {
                    return Err(E::custom(
                        "audit limit must be positive and representable in milliseconds",
                    ));
                }
            ...
            fn visit_i64<E: de::Error>(self, n: i64) -> Result<Self::Value, E> {
                self.visit_u64(
                    u64::try_from(n).map_err(|_| E::custom("audit limit must be positive"))?,
                )
            }
            fn visit_str<E: de::Error>(self, value: &str) -> Result<Self::Value, E> {
                match value {
                    ...
                    _ => Err(E::custom(
                        "audit limit string must be exactly \"unlimited\"",
                    )),
                }
            }
        }
...
impl RepositoryAuditConfig {
    pub fn resolve(&self, host_call_timeout_secs: u32) -> ResolvedRepositoryAuditConfig {
        ...
        for field in [
            ...
            sources.entry(field.into()).or_insert_with(|| {
        ...
        ResolvedRepositoryAuditConfig {
            sources,
            attempt_timeout_secs: self
                .attempt_timeout_secs
                .unwrap_or(AuditLimit::Finite(u64::from(host_call_timeout_secs))),
...
pub(crate) fn record_audit_sources(
    ...
    for field in [
        ...
        let section = if field == "host_call_timeout_secs" {
            "generated"
        } else {
            "repository_audit"
        };
