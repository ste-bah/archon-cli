...
async fn main_request_tool_definitions_are_byte_stable_across_turns() {
    ...
    let first_tools =
        archon_llm::providers::OpenAiProvider::map_tools_to_openai(&first.request.tools);
    let second_tools =
        archon_llm::providers::OpenAiProvider::map_tools_to_openai(&second.request.tools);
    assert_eq!(
...
async fn eight_tool_rounds_send_trimmed_history_but_reopen_with_full_results() {
    ...
    for (round, result) in full_results.iter().enumerate() {
        ...
        agent
            .state
            .messages
            .push(serde_json::json!({"role":"assistant","content":[{
                "type":"tool_use","id":format!("tool-{round}"),"name":"Bash","input":{}
            }]}));
        agent
            .state
            .messages
            .push(serde_json::json!({"role":"user","content":[{
