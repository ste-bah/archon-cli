...
mod tests {
    ...
    fn general_purpose_allows_all_tools() {
        let agents = get_built_in_agents();
        let gp = agents.iter().find(|a| a.agent_type == "general-purpose").unwrap();
        assert!(gp.allowed_tools.is_none(), "general-purpose should allow all tools (None)");
    }
    ...
    fn fork_agent_inherits_all_tools() {
        let agents = get_built_in_agents();
        let fork = agents.iter().find(|a| a.agent_type == "fork").unwrap();
        assert!(fork.allowed_tools.is_none(), "fork should allow all tools (None)");
    }
    ...
    fn fork_agent_inherits_parent_model() {
        let agents = get_built_in_agents();
        let fork = agents.iter().find(|a| a.agent_type == "fork").unwrap();
        assert!(fork.model.is_none(), "fork should inherit parent model (None)");
    }
    ...
    fn fork_agent_omits_claude_md() {
        let agents = get_built_in_agents();
        let fork = agents.iter().find(|a| a.agent_type == "fork").unwrap();
        assert!(fork.omit_claude_md, "fork should omit CLAUDE.md (parent has it)");
    }
    ...
    fn fork_agent_system_prompt_contains_boilerplate_marker() {
        let agents = get_built_in_agents();
        let fork = agents.iter().find(|a| a.agent_type == "fork").unwrap();
        assert!(fork.system_prompt.contains("<fork_boilerplate>"),
    ...
    fn is_fork_enabled_defaults_to_false() {
        // Clean env state
        unsafe { std::env::remove_var("ARCHON_FORK_SUBAGENT"); }
        assert!(!is_fork_enabled());
    }

    #[test]
    fn is_fork_enabled_true_when_set() {
        unsafe { std::env::set_var("ARCHON_FORK_SUBAGENT", "1"); }
        assert!(is_fork_enabled());
        unsafe { std::env::remove_var("ARCHON_FORK_SUBAGENT"); }
    }
    ...
    fn is_fork_enabled_true_when_set_to_true() {
        unsafe { std::env::set_var("ARCHON_FORK_SUBAGENT", "true"); }
        assert!(is_fork_enabled());
        unsafe { std::env::remove_var("ARCHON_FORK_SUBAGENT"); }
    }
    ...
    fn is_fork_enabled_false_for_zero() {
        unsafe { std::env::set_var("ARCHON_FORK_SUBAGENT", "0"); }
        assert!(!is_fork_enabled());
        unsafe { std::env::remove_var("ARCHON_FORK_SUBAGENT"); }
    }
