...
impl Capture {
    ...
    pub fn push(&mut self, bytes: &[u8]) {
        ...
        self.first
            .extend_from_slice(&bytes[..bytes.len().min(500 - self.first.len())]);
        if bytes.len() >= 500 {
            self.last = bytes[bytes.len() - 500..].to_vec();
        } else {
            self.last.extend_from_slice(bytes);
            if self.last.len() > 500 {
                self.last.drain(..self.last.len() - 500);
            }
        }
...
pub(crate) fn stream(
    ...
    secrets: Vec<String>,
) -> impl Stream<Item = Result<impl AsRef<[u8]>, reqwest::Error>> + Send + Unpin + 'static {
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        match Pin::new(&mut this.inner).poll_next(cx) {
            Poll::Ready(Some(Ok(bytes))) => {
                this.capture.push(bytes.as_ref());
                Poll::Ready(Some(Ok(bytes)))
            }
            Poll::Ready(Some(Err(error))) => {
                this.capture.end = "network_error";
                Poll::Ready(Some(Err(error)))
            }
            Poll::Ready(None) => {
                this.capture.decode_line();
                this.capture.end = "eof";
                Poll::Ready(None)
            }
