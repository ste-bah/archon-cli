...
use regex::Regex;
static FIELDS: Lazy<Regex> = Lazy::new(|| Regex::new(
...
pub(super) fn scrub(text: &str, secrets: &[String], left_cut: bool, right_cut: bool) -> String {
    ...
    for secret in secrets.iter().filter(|s| !s.is_empty()) {
        if right_cut {
            let n = (1..secret.len()).rev().find(|&n| secret.is_char_boundary(n) && text.ends_with(&secret[..n]));
            if let Some(n) = n { text.truncate(text.len()-n); text.push_str("[REDACTED]"); }
        }
        text = text.replace(secret, "[REDACTED]");
    }
    let text = FIELDS.replace_all(&text, "$1[REDACTED]\"");
    static SHAPES: Lazy<Regex> = Lazy::new(|| Regex::new(
