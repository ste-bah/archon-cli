...
// ---------------------------------------------------------------------------
// Tool use over ConverseStream
//
// The event carries the block under `start`, NOT `contentBlock` - the latter is
// the non-streaming Converse response's name. Reading the wrong one matched
// nothing, so every tool-use block was announced as plain text with no id and
// no name and no call could be assembled: the model answered prose fine and did
// nothing whenever a tool was required. There was no test over this event at
