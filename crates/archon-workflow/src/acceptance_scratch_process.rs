...
impl Drop for GroupGuard {
    fn drop(&mut self) {if self.0>0 {unsafe {libc::kill(-self.0,libc::SIGKILL);}}}
}
async fn drain(mut pipe:impl AsyncRead+Unpin,limit:usize,overflow:Arc<AtomicBool>)->std::io::Result<Vec<u8>> {
    let mut retained=Vec::new();let mut buffer=[0;8192];
    loop {let n=pipe.read(&mut buffer).await?;if n==0 {return Ok(retained);}
        let keep=n.min(limit.saturating_sub(retained.len()));retained.extend_from_slice(&buffer[..keep]);
        if keep<n {overflow.store(true,Ordering::SeqCst);}
    }
}
fn scratch_size(path:&Path)->std::io::Result<u64> {
    let m=std::fs::symlink_metadata(path)?;
    if m.is_dir() {let mut n=0u64;for entry in std::fs::read_dir(path)? {n=n.saturating_add(scratch_size(&entry?.path())?);}Ok(n)}
...
pub(super) async fn run(
    ...
    process.arg("-s").current_dir(cwd).env_clear().envs(roots.environment(policy))
    ...
    let mut group=GroupGuard(child.id().ok_or_else(||invalid("scratch child has no process id"))? as i32);
    let overflow=Arc::new(AtomicBool::new(false));
    let stdout=tokio::spawn(drain(child.stdout.take().unwrap(),policy.output_bytes,overflow.clone()));
    let stderr=tokio::spawn(drain(child.stderr.take().unwrap(),policy.output_bytes,overflow.clone()));
    let mut stdin=child.stdin.take().unwrap();let bytes=command.bytes().to_vec();
    let writer=tokio::spawn(async move {stdin.write_all(&bytes).await?;stdin.write_all(b"\n").await?;stdin.shutdown().await});
    ...
    let pipes=tokio::time::timeout(Duration::from_secs(3),async {
        let out=stdout.await.map_err(|e|invalid(e.to_string()))?.map_err(|e|WorkflowError::io(cwd,e))?;
        let err=stderr.await.map_err(|e|invalid(e.to_string()))?.map_err(|e|WorkflowError::io(cwd,e))?;
        Ok::<_,WorkflowError>((out,err))
    }).await.map_err(|_|invalid("scratch child pipes did not close after group teardown"))??;
    // group is disarmed only after ESRCH, never merely because the leader exited.
    for _ in 0..100 {
        if unsafe{libc::kill(-group.0,0)}==-1 && std::io::Error::last_os_error().raw_os_error()==Some(libc::ESRCH) {group.0=0;break;}
