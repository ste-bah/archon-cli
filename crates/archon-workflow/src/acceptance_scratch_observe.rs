...
pub async fn observe_commands_cancellable(
    ...
    for (reference, command) in refs.iter().zip(commands) {
        if reference.kind == crate::acceptance_world::AcceptanceCommandKind::NestedVerifier {
            ...
            if let crate::task_set_contract::AcceptanceCheck::Floor { contract: floor } =
                &entry.check
            {
                ...
                match roots.source_inventory() {
                    ...
                    _ => {
                        result.operational_error =
                            Some("native scratch source changed; warm target reuse refused".into())
                    }
                }
                let stop = result.operational_error.is_some();
                checks.push(result);
                if stop {
                    break;
                }
