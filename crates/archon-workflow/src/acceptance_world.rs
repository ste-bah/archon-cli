...
use crate::task_set_contract::{
    AcceptanceCheck, AcceptanceContract, JudgeDecision, TrustedCwd,
    acceptance_policy_findings, content_digest, validate_acceptance_structure,
};
...
impl AuthorizedCommand {
    ...
    pub fn cwd(&self) -> TrustedCwd { self.cwd }
}
...
pub fn resolve_command(
    ...
    let expected = contract.acceptance.iter().map(|entry| entry.id.clone()).collect();
    ...
    if acceptance_policy_findings(contract).iter().any(|finding| finding.field.starts_with(&prefix)) {
    ...
    let (command, cwd) = match (&entry.check, reference.kind) {
        ...
        (AcceptanceCheck::Floor { contract }, AcceptanceCommandKind::NestedVerifier) =>
            (contract.typed_verifier_command.as_deref()
                .ok_or_else(|| reject("floor has no pinned verifier command"))?, TrustedCwd::ProjectRoot),
        (_, AcceptanceCommandKind::ResidualFailClosed) => return Err(reject(
            "residual fail_closed_check has no pinned judged binding; refusing native execution")),
        _ => return Err(reject("acceptance command kind does not match pinned check")),
    };
    if content_digest(command.as_bytes()) != reference.command_digest {
        return Err(reject("acceptance command digest changed"));
    }
    Ok(AuthorizedCommand { bytes: command.as_bytes().to_vec(), cwd })
}
