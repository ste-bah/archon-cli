...
fn copy_tree_inner(
    ...
    if meta.is_dir() {
        ...
        if meta.len() > *remaining {
            return Err(invalid("scratch copy exceeds configured size limit"));
        }
        ...
        std::fs::set_permissions(dest, meta.permissions())
            .map_err(|e| WorkflowError::io(dest, e))?;
        *remaining -= bytes.len() as u64;
    } else {
        return Err(invalid(format!(
            "nonregular or symlink snapshot input refused: {}",
            source.display()
        )));
    }
    Ok(())
...
pub(super) fn remove_owned_tree(path: &Path) -> WorkflowResult<()> {
    ...
    if meta.is_dir() && !meta.file_type().is_symlink() {
        ...
        for item in std::fs::read_dir(path).map_err(|e| WorkflowError::io(path, e))? {
            remove_owned_tree(&item.map_err(|e| WorkflowError::io(path, e))?.path())?;
        }
        std::fs::remove_dir(path).map_err(|e| WorkflowError::io(path, e))
    } else {
        std::fs::remove_file(path).map_err(|e| WorkflowError::io(path, e))
    }
}
...
pub fn inventory(root: &Path) -> WorkflowResult<BTreeMap<String, String>> {
    fn visit(root: &Path, path: &Path, out: &mut BTreeMap<String, String>) -> WorkflowResult<()> {
        ...
        let key = path
            .strip_prefix(root)
            .map_err(|_| invalid("inventory escaped root"))?
            .to_string_lossy()
            .into_owned();
