...
fn sealed_source_does_not_follow_live_head_or_dirty_edits() {
    ...
    assert_eq!(
        std::fs::read_to_string(workspace.plan.isolated_root.join("src/lib.rs")).unwrap(),
        "captured dirty\n"
    );
    assert_eq!(
...
fn two_branches_share_captured_untracked_bytes_without_live_reread() {
    ...
    for workspace in [&first, &second] {
        assert_eq!(
            std::fs::read_to_string(workspace.plan.isolated_root.join("src/new.txt")).unwrap(),
            "shared source\n"
        );
    }
    assert_eq!(
        run_git(&["rev-parse", "HEAD^{tree}"], &first.plan.isolated_root)
            .unwrap()
            .stdout,
        run_git(&["rev-parse", "HEAD^{tree}"], &second.plan.isolated_root)
            .unwrap()
            .stdout
    );
}
...
fn sealed_source_captures_untracked_inputs_without_language_filtering() {
    ...
    let root = repo.path();
    std::fs::write(
        root.join("rules.custom-language"),
        "behavior outside declared target\n",
    )
    .unwrap();
    let plan = plan_for(root, &["src/lib.rs"]);
    let sealed = capture_sealed_source(root, &plan, &default_cfg()).unwrap();
    ...
    assert_eq!(
        std::fs::read_to_string(workspace.plan.isolated_root.join("rules.custom-language"))
            .ok()
            .as_deref(),
...
fn sealed_source_rejects_oversized_untracked_context_instead_of_omitting_it() {
    ...
    assert!(
        matches!(
            capture_sealed_source(root, &plan, &cfg),
            Err(IsolationError::FileTooLarge { .. })
        ),
...
fn sealed_source_preserves_untracked_support_executable_mode() {
    ...
    let workspace = sealed.assessment_workspace(root, &plan).unwrap();
    assert_ne!(
        std::fs::metadata(workspace.plan.isolated_root.join("driver.sh"))
            .unwrap()
            .permissions()
            .mode()
            & 0o111,
...
fn sealed_capture_rejects_source_changed_between_overlay_and_metadata_reads() {
    ...
    git(
        &["config", "diff.capture.command", gate.to_str().unwrap()],
        root,
    );
...
fn sealed_materialization_refuses_overlay_metadata_disagreement() {
    ...
    sealed.baseline.declared_target_meta.insert(
        "src/lib.rs".into(),
        file_meta(&root.join("src/lib.rs")).unwrap(),
    );
    let error = sealed
        .assessment_workspace(root, &plan)
        .expect_err("inconsistent overlay and metadata were sealed as an assessable snapshot")
        .to_string();
    assert!(
        error.contains("src/lib.rs")
            && error.contains("obligated by plan: true")
...
fn sealed_assessment_omits_ignored_untracked_metadata_declared_or_not() {
    ...
    let workspace = source
        .assessment_workspace(root, &plan)
        .expect("undeclared ignored metadata is not a snapshot violation");
    ...
    let workspace = source
        .assessment_workspace(root, &declared_plan)
        .expect("a declared path the repository ignores cannot be sealed, so it is omitted");
    assert!(!workspace.plan.isolated_root.join("docs/report.md").exists());
}
...
fn sealed_assessment_survives_declared_target_under_ignored_directory() {
    ...
    assert!(
        source.baseline.declared_target_meta[artifact].exists,
        "capture records the canonical file"
    );
    let workspace = source
        .assessment_workspace(root, &plan)
    ...
    source.baseline.declared_target_meta.insert(
        ".archon/hooks.toml".into(),
        file_meta(&root.join(".archon/hooks.toml")).unwrap(),
    );
    let error = source
        .assessment_workspace(root, &plan)
        .unwrap_err()
        .to_string();
    assert!(
        error.contains(".archon/hooks.toml")
            && error.contains("obligated by plan: true")
            && error.contains("content hash"),
        "{error}"
    );
...
fn sealed_validation_does_not_consult_git_in_the_materialized_root() {
    ...
    source
        .validate_materialized(plain.path(), &plan)
        .expect("a plain directory holding the sealed files validates");
