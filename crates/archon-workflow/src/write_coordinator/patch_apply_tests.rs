...
use crate::write_coordinator::{ItemId, WritePlan, WriteCoordinatorConfig};
...
fn prepare(
    ...
    let plan = WritePlan {
        ...
        target_files: declared.iter().map(|t| normalize_target(t, repo).unwrap()).collect(),
    ...
    let json = std::fs::read_to_string(
        run_root.join("write-coordination/stages/impl/manifests").join(format!("{item}.json")),
    )
    .unwrap();
...
fn single_item_clean_apply() {
    ...
    let (m, pre) = prepare(repo.path(), "impl-0", &["src/lib.rs"], &[("src/lib.rs", "// edited\n")]);
    let mut pre_by_item = BTreeMap::new();
    pre_by_item.insert(m.item_id.clone(), pre);
    let rec = apply_wave(repo.path(), &[m.clone()], &pre_by_item, 0, &run_root_of(repo.path()), "run1", "impl")
        .expect("apply");
    assert_eq!(rec.items_applied, vec!["impl-0".to_string()]);
    assert!(rec.items_failed.is_empty());
    assert_eq!(std::fs::read_to_string(repo.path().join("src/lib.rs")).unwrap(), "// edited\n");
    assert_eq!(resume_status(&m.item_id, &run_root_of(repo.path()), "impl"), ApplyResumeStatus::Applied);
}

#[test]
fn two_disjoint_items_both_apply() {
    let repo = canonical_repo();
    let (m0, p0) = prepare(repo.path(), "impl-0", &["src/lib.rs"], &[("src/lib.rs", "// a\n")]);
    let (m1, p1) = prepare(repo.path(), "impl-1", &["src/other.rs"], &[("src/other.rs", "// b\n")]);
    let mut pre = BTreeMap::new();
    pre.insert(m0.item_id.clone(), p0);
    pre.insert(m1.item_id.clone(), p1);
    let rec = apply_wave(repo.path(), &[m1, m0], &pre, 0, &run_root_of(repo.path()), "run1", "impl")
        .expect("apply");
    // Sorted by item_id: impl-0 before impl-1.
    assert_eq!(rec.items_applied, vec!["impl-0".to_string(), "impl-1".to_string()]);
    assert!(rec.items_failed.is_empty());
}
...
fn stale_baseline_unchanged_file_ignored() {
    ...
    let rec = apply_wave(repo.path(), &[m], &pre_by_item, 0, &run_root_of(repo.path()), "run1", "impl")
        .expect("apply");
    assert_eq!(rec.items_applied, vec!["impl-0".to_string()]);
}
...
fn patch_conflict_cleans_canonical() {
    let repo = canonical_repo();
    let (m, _pre) = prepare(repo.path(), "impl-0", &["src/lib.rs"], &[("src/lib.rs", "// edited\n")]);
    ...
    hashes.insert("src/lib.rs".to_string(), blake3_of(&repo.path().join("src/lib.rs")));
...
fn failing_verify_reports_nonzero_no_rollback() {
    ...
    let mut pre_by_item = BTreeMap::new();
    pre_by_item.insert(m.item_id.clone(), pre);
    apply_wave(repo.path(), &[m], &pre_by_item, 0, &run_root_of(repo.path()), "run1", "impl")
        .expect("apply");
    let verify = run_wave_verify(repo.path(), Some("exit 3"), 0, &run_root_of(repo.path()), "impl")
        .expect("verify runs");
...
fn conflict_graph_violation_before_any_git() {
    let repo = canonical_repo();
    let (mut m0, _) = prepare(repo.path(), "impl-0", &["src/lib.rs"], &[("src/lib.rs", "// a\n")]);
    let (mut m1, _) = prepare(repo.path(), "impl-1", &["src/lib.rs"], &[("src/lib.rs", "// b\n")]);
    m0.declared_target_files = vec!["src/lib.rs".into()];
    m1.declared_target_files = vec!["src/lib.rs".into()];
    let pre = BTreeMap::new();
    match apply_wave(repo.path(), &[m0, m1], &pre, 0, &run_root_of(repo.path()), "run1", "impl") {
    ...
    // Canonical untouched.
    assert_eq!(std::fs::read_to_string(repo.path().join("src/lib.rs")).unwrap(), "// original\n");
}

#[test]
fn apply_wave_does_not_mutate_caller_slice() {
    let repo = canonical_repo();
    let (m0, p0) = prepare(repo.path(), "impl-1", &["src/lib.rs"], &[("src/lib.rs", "// a\n")]);
    let (m1, p1) = prepare(repo.path(), "impl-0", &["src/other.rs"], &[("src/other.rs", "// b\n")]);
    let mut pre = BTreeMap::new();
    pre.insert(m0.item_id.clone(), p0);
    pre.insert(m1.item_id.clone(), p1);
    let slice = vec![m0, m1];
    let order_before: Vec<String> = slice.iter().map(|m| m.item_id.clone()).collect();
    apply_wave(repo.path(), &slice, &pre, 0, &run_root_of(repo.path()), "run1", "impl").expect("apply");
    let order_after: Vec<String> = slice.iter().map(|m| m.item_id.clone()).collect();
    assert_eq!(order_before, order_after, "caller slice order must be unchanged");
}
...
fn lock_path_is_exact() {
    ...
    let expected = root.join(".archon/workflows/write-locks").join(format!("{hex}.lock"));
    assert_eq!(lock_path_for(root), expected);
}

#[test]
fn distinct_repos_distinct_lock_paths() {
    assert_ne!(lock_path_for(Path::new("/a")), lock_path_for(Path::new("/b")));
}

#[test]
fn resume_status_variants() {
    let repo = canonical_repo();
    let (m, _) = prepare(repo.path(), "impl-0", &["src/lib.rs"], &[("src/lib.rs", "// a\n")]);
    let rr = run_root_of(repo.path());
    assert_eq!(resume_status(&ItemId::from("ghost"), &rr, "impl"), ApplyResumeStatus::NotPersisted);
    assert_eq!(resume_status(&m.item_id, &rr, "impl"), ApplyResumeStatus::PendingApply);
    persist_status(&rr, &m, ManifestStatus::Applied);
    assert_eq!(resume_status(&m.item_id, &rr, "impl"), ApplyResumeStatus::Applied);
    persist_status(&rr, &m, ManifestStatus::IdempotentNoop);
    assert_eq!(resume_status(&m.item_id, &rr, "impl"), ApplyResumeStatus::IdempotentNoop);
    persist_status(&rr, &m, ManifestStatus::Conflicted);
    assert_eq!(resume_status(&m.item_id, &rr, "impl"), ApplyResumeStatus::Conflicted);
    persist_status(&rr, &m, ManifestStatus::Failed { reason: "boom".into() });
...
fn utf8_tail_no_invalid_bytes_on_boundary() {
    // 4095 'a' then a 3-byte char (e2 82 ac = €) => boundary splits the char.
...
fn map_apply_error_carries_item() {
    ...
    let err = map_apply_error_with_item(
        &item,
        IsolationError::ProcessFailed { stderr: "boom".into() },
    );
    match err {
...
fn lock_blocks_then_succeeds() {
    ...
    assert!(start.elapsed() >= Duration::from_millis(150), "should have waited for holder");
    holder.join().unwrap();
}
...
fn lock_times_out_when_held() {
    ...
    assert!(matches!(res, Err(ApplyError::LockTimeout { .. })), "got {res:?}");
    done_tx.send(()).unwrap();
    holder.join().unwrap();
}
