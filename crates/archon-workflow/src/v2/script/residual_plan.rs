...
use super::remediation_escalation::unit_task_ids;
use super::residual_paths::{TaskTexts, expandable, named_files_at, owners, related_tasks};
use super::{
...
fn gaps_of(record: &WorkflowV2CallRecord) -> Vec<(String, String, Option<String>)> {
...
pub fn residuals_of(record: &WorkflowV2CallRecord, root: Option<&Path>) -> Vec<Residual> {
    let unit = remediation_contract(&record.call)
        .map(unit_task_ids)
        .unwrap_or_default();
    let judged = super::remediation_escalation::judged_commit(&record.result);
    gaps_of(record)
        .into_iter()
...
fn flagged_severity(description: &str) -> Option<&str> {
