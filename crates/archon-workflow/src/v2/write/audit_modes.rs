...
pub(super) async fn run(
    ctx: WriteFanoutContext<'_>, branches: Vec<WorkflowV2FanoutItem>,
    mut plan: WorkflowV2WritePlan, reused: Vec<WorkflowV2Result>,
    ...
    for wave in &mut plan.waves {
        for assignment in &mut wave.assignments {
            assignment.worktree_path = Some(ctx.v2_store.root().join("worktrees")
                .join(sanitize_v2_path_segment(&ctx.execution.call.id))
                .join(sanitize_v2_path_segment(&assignment.item_id)).display().to_string());
        }
    }
