...
pub(super) async fn refresh(
    branches: &mut [WorkflowV2FanoutItem], items: &[WorkflowV2WriteItem],
    root: Option<&str>, call_id: &str, store: &WorkflowV2ResultStore,
    dispatch: &dyn WorkflowAgentDispatch,
) -> WorkflowResult<()> {
    let Some(audit) = dispatch.repository_audit() else { return Ok(()); };
    ...
    if !has_cached_candidate { return Ok(()); }
    let Some(root) = root else { return Ok(()); };
    let mut paths = audit.state()?.declared_paths;
    paths.extend(items.iter().flat_map(|item| item.owned_targets.iter().cloned()));
