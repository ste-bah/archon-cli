...
fn digest(path:&Path)->Option<String> {
    ...
    std::fs::read(path).ok().map(|bytes|crate::task_set_contract::content_digest(&bytes))
}
impl ArtifactDelivery {
    ...
    pub fn stamp(&self,result:&mut WorkflowV2Result,repository_changed:bool){
        if !result.data.is_object(){result.data=serde_json::json!({});}
        ...
        let after=self.paths.iter().map(|(raw,path)|(raw.clone(),digest(path))).collect::<BTreeMap<_,_>>();
        let changed=after.iter().filter(|(key,value)|self.before.get(*key)!=Some(value)).map(|(key,_)|key.clone()).collect::<Vec<_>>();
        ...
        if result.status==WorkflowV2Status::Accepted && !repository_changed && changed.is_empty()
            && self.paths.iter().all(|(raw,_)|after.get(raw).is_some_and(Option::is_some)) {
