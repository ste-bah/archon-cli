...
fn preserved_patch_commits_through_production_apply_wrapper() {
    ...
    let clone = std::process::Command::new("git")
        .args(["clone", "--quiet", "--no-hardlinks", "--shared"])
        .arg(&source)
        .arg(&repo)
        .status()
        .unwrap();
    ...
    let targets = String::from_utf8(names.stdout)
        .unwrap()
        .lines()
        .map(|line| normalize_target(line.splitn(3, '\t').nth(2).unwrap(), &repo).unwrap())
        .collect::<Vec<_>>();
    assert!(
        !targets.is_empty(),
        "numstat must select preserved patch paths"
    );
    let run_root = temp.path().join("run");
    let item = "preserved-item";
    let plan = WritePlan {
        run_id: "replay".into(),
        stage_id: "preserved-wave".into(),
        item_id: item.into(),
        canonical_root: repo.clone(),
        isolated_root: temp.path().join("workspace"),
    ...
    let ctx = WorktreePlanRunContext {
        task: "replay preserved patch",
        target_repository_root: repo.to_str(),
        execution: &execution,
        adapter: WorkflowV2AgentAdapter::new(),
        dispatch: &UnusedDispatch,
        v2_store: &v2,
        store_for_control: &store,
        run_id: "replay",
        setup: &setup,
    ...
    let mut artifacts = WorktreeWaveArtifacts {
        manifests: vec![manifest],
        pre_hashes: BTreeMap::from([(item.into(), captured.pre_hashes.clone())]),
        ..Default::default()
    };
