...
/// string and character literal contents too (`code_hygiene::source_text`).
/// Like the line cap it is a
...
fn complexity_rule(max: u32) -> String {
    ...
    format!(
        "- Function complexity: at most {max} per function, scored as 1 plus one for every {} \
         ...
         string and character literal contents in brace languages, excluded). A function already over the cap may be changed only if \
