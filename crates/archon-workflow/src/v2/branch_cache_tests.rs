...
// ---------------------------------------------------------------------------
// Issue-24: reuse keys on the item as authored. The landed short-circuit is
// tested beside this in `branch_cache_landed_tests.rs`, which shares the
// helpers below.
// ---------------------------------------------------------------------------
...
pub(super) fn authored(target_files: &[&str], prompt: &str) -> WorkflowV2FanoutItem {
...
pub(super) fn first_item() -> WorkflowV2FanoutItem {
    prepared(authored(&["src/lib.rs"], "add the parser"), 120)
}

/// An accepted outcome saved the way the write layer saves it now.
pub(super) fn saved_outcome(item: &WorkflowV2FanoutItem) -> WorkflowV2BranchOutcome {
    let mut outcome = accepted_outcome(item);
    outcome.item_input_hash = Some(reuse_identity(item));
    outcome
}

pub(super) fn split(store: &WorkflowV2ResultStore, item: WorkflowV2FanoutItem) -> (usize, usize) {
