...
fn accepted_outcome(commands: Vec<WorkflowV2CommandRecord>, data: serde_json::Value) -> WorkflowV2BranchOutcome {
...
fn a_red_test_not_owned_by_another_task_refuses_the_accepted_verdict_whatever_the_prose_says() {
    ...
    assert!(result.residual_gaps.iter().any(|g| g.id == BASELINE_RED_TEST_GAP_ID), "{result:?}");
    assert_eq!(result.data["baseline_red_tests"], json!(["grant::tests::mine"]));
    assert_eq!(result.data["verification_failure_class"], json!("actionable_verification_failure"));
}
...
fn a_red_test_on_the_other_owner_or_ignore_list_keeps_the_verdict() {
    ...
    let mut outcomes = vec![accepted_outcome(
        vec![command("cargo test -p engine grant", WorkflowV2CommandStatus::Failed, output, true)],
        json!({}),
    )];
    enforce_baseline_tests(&mut outcomes, &by_item());
    assert_eq!(outcomes[0].status, WorkflowV2Status::Accepted);
    assert!(outcomes[0].result.as_ref().unwrap().residual_gaps.is_empty());
}

#[test]
fn the_typed_failed_names_are_read_when_the_output_names_nothing() {
    let mut outcomes = vec![accepted_outcome(
        vec![command("cargo test -p engine grant", WorkflowV2CommandStatus::Succeeded, "ok", false)],
...
fn a_pre_existing_claim_on_a_declared_command_that_names_no_test_is_not_accepted() {
    ...
    assert_eq!(outcomes[0].status, WorkflowV2Status::NeedsReview);
    assert_eq!(result.data["baseline_unproven_pre_existing"], json!(["cargo test -p engine grant"]));
}
...
fn an_item_without_a_stamp_and_a_non_accepted_outcome_are_left_alone() {
    ...
    let mut outcomes = vec![accepted_outcome(
        vec![command("cargo test -p engine grant", WorkflowV2CommandStatus::Failed, red, false)],
        json!({}),
    )];
...
fn the_stamp_is_assembled_from_the_task_records_and_routed_findings_and_lands_on_the_item() {
    ...
    record(&store, "agents-1", "agents-1-a", "1111111111111111", "TASK-A");
    route_finding(&store, "TASK-A", json!({"test_id": "other::routed_in", "file": "x.rs", "canonical_task_ids": ["TASK-A"]}));
    let stamp = BaselineStamp::for_tasks(&store, &["TASK-A".to_string()]).expect("stamp");
    assert_eq!(stamp.base_commit, "1111111111111111");
    assert_eq!(stamp.must_pass, vec!["grant::tests::mine".to_string(), "other::routed_in".to_string()]);
    assert_eq!(stamp.other_owner[0].owner_task, "TASK-B");
    assert_eq!(stamp.declared_commands, vec!["cargo test -p engine grant".to_string()]);
    assert!(BaselineStamp::for_tasks(&store, &["TASK-Z".to_string()]).is_none());

    let mut input = json!({"item": {"item_id": "verify-1-check", "canonical_task_ids": ["TASK-A"]}});
    ...
    assert!(other.get(BASELINE_TESTS_INPUT_KEY).is_none(), "only verification calls are stamped");
    ...
    assert_eq!(by_item.get("verification-wave-verify-1-verify-1-check"), Some(&stamp));
}
