...
pub(super) fn attach(input:&mut Value, universe:&WorkflowV2TaskUniverse) {
    ...
    let tasks=universe.tasks.iter().map(|task|(task.canonical_task_id.as_str(),task)).collect::<BTreeMap<_,_>>();
    let mut pending=claimed.clone();let mut visited=BTreeSet::new();let mut paths=BTreeSet::new();
    while let Some(id)=pending.pop() {
        if !visited.insert(id.clone()){continue;}
        let Some(task)=tasks.get(id.as_str()) else {continue;};
    ...
    let project=input.get("_workflow_project_artifact_policy").and_then(|p|p.get("project_root")).cloned();
    ...
    match input {
        Value::Object(map)=>{map.insert("dependency_read_context".into(),context);},
