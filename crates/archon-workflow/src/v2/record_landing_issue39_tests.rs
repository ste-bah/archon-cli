...
use super::record_landing::{RecordKind, RecordLanding, VERIFY_VERDICTS, enum_names, schema_hint};
...
fn evidence() -> Value {
    json!([{"kind":"test","summary":"ran the suite"}])
}
...
fn skeleton_contract_with_blank_kind_or_path_is_rejected_by_path() {
    ...
    let error = landing
        .land(skeleton_task(
            json!([]),
            json!([{"kind":"","artifact_path":"x"}]),
        ))
        .unwrap_err()
        .to_string();
    assert!(
        error.contains("task.deliverable_contracts[0].kind"),
        "{error}"
    );
    ...
    assert!(
        error.contains("task.deliverable_contracts[1].artifact_path"),
        "{error}"
    );
    assert!(error.contains("Expected schema:"), "{error}");
    assert!(
...
fn verify_pass_without_a_succeeded_command_is_rejected() {
    ...
    assert!(
        ...
        "a skipped entry is not a pass: {error}"
    );
    ...
    assert!(
        error.contains(expected),
        "a blank output_summary is synthesised, not captured: {error}"
    );
    ...
    assert!(
        error.contains(expected),
        "a blank command is not a command: {error}"
    );
...
fn verify_pass_with_a_succeeded_command_lands_and_a_failure_needs_none() {
    ...
    assert_eq!(
        landing.assemble(&json!({"records_landed":1})).unwrap()["verification_records"][0]["status"],
        "failed"
    );
}
...
fn commands_run_is_normalised_like_the_envelope_before_deserialising() {
    ...
    let command = &landing.assemble(&json!({"records_landed":1})).unwrap()["verification_records"]
        [0]["commands_run"][0];
    ...
    assert!(
        summary.starts_with(crate::v2::agent_output_normalize::SYNTHESIZED_OUTPUT_SUMMARY_PREFIX),
        "{summary}"
    );
    let error = landing
        .land(json!({"subject":"S","evidence":evidence(),"commands_run":[{"command":"run-suite"}]}))
        .unwrap_err()
        .to_string();
    assert!(
        error.contains("invalid record at commands_run[0].status: missing field `status`"),
        "no exit_code, nothing to derive from: {error}"
    );
    let error = landing
...
fn synthesised_output_summary_does_not_satisfy_a_verify_pass() {
    ...
    assert!(
        error.contains(
            "requires at least one succeeded commands_run entry with a captured output_summary"
        ),
        "{error}"
    );
}
...
fn records_landed_mismatch_names_both_counts() {
    let (_temp, landing) = open(RecordKind::Review, &["S"]);
    landing
        .land(json!({"subject":"S","summary":"looked","evidence":evidence()}))
        .unwrap();
    let error = landing
        .assemble(&json!({"records_landed":3}))
        .unwrap_err()
        .to_string();
    assert!(
        error.contains("records_landed=3 but host retained 1 record(s)"),
        "{error}"
    );
    let error = landing.assemble(&json!({})).unwrap_err().to_string();
    assert!(
        error.contains("records_landed=missing but host retained 1 record(s)"),
        "{error}"
    );
}
...
fn enum_names_are_the_lists_the_schema_hint_prints() {
    ...
    assert_eq!(
        names.verify_verdicts,
        vec!["accepted", "noop", "failed", "blocked", "needs_review"]
    );
    ...
    assert!(
        names
            .verify_verdicts
            .iter()
            .all(|v| names.statuses.contains(v))
    );
}
