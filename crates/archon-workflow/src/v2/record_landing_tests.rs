...
fn reduce(subject: &str) -> (tempfile::TempDir, RecordLanding) {
    open(&[subject], false)
}
...
fn evidence() -> serde_json::Value {
    json!([{"kind":"inspection","summary":"read the implementation"}])
}
...
fn missing_nested_field_names_its_path_and_the_schema() {
    ...
    let error = landing
        .land(json!({"subject":"S","evidence":[{"summary":"x"}]}))
        .unwrap_err()
        .to_string();
    assert!(
        error.contains("invalid record at evidence[0].kind: missing field `kind`"),
        "{error}"
    );
    assert!(
        error.contains("Expected schema: {subject: string"),
        "{error}"
    );
    ...
    assert!(
        error.contains(
            "status?: pending|running|accepted|noop|failed|blocked|needs_review|cancelled"
        ),
        "{error}"
    );
}
...
fn deeper_missing_field_and_top_level_missing_subject_are_both_located() {
    ...
    assert!(
        error.contains("invalid record at commands_run[0].command: missing field `command`"),
        "{error}"
    );
    let error = landing
        .land(json!({"evidence":[{"kind":"test","summary":"ran"}]}))
        .unwrap_err()
        .to_string();
    assert!(
        error.contains("invalid record at subject: missing field `subject`"),
        "{error}"
    );
...
fn unknown_top_level_key_is_named_with_the_schema() {
    let (_temp, landing) = review(&["S"]);
    let error = landing
        .land(json!({"subject":"S","kind":"review","evidence":[{"kind":"test","summary":"ran"}]}))
        .unwrap_err()
        .to_string();
    assert!(
        error.contains("invalid record at kind: unknown field `kind`"),
        "{error}"
    );
    assert!(error.contains("Expected schema:"), "{error}");
...
fn validation_rejections_keep_their_message_and_append_the_schema() {
    let (_temp, landing) = review(&["S"]);
    let error = landing
        .land(json!({"subject":"other","evidence":[{"kind":"test","summary":"ran"}]}))
        .unwrap_err()
        .to_string();
    assert!(
        error.contains("record subject is outside this call. Expected schema: {subject"),
        "{error}"
    );
    let error = landing
        .land(json!({"subject":"S"}))
        ...
        .to_string();
    assert!(
        error.contains("record requires concrete evidence. Expected schema: {subject"),
        "{error}"
    );
}
...
fn relanding_unions_by_default_and_replace_supersedes() {
    ...
    assert_eq!(
        data["verification_records"][0]["evidence"]
            .as_array()
            .unwrap()
            .len(),
        2
    ...
    assert_eq!(
        data["verification_records"][0]["evidence"],
        json!([{"kind":"inspection","summary":"looked at f3"}])
    );
    assert_eq!(data["verification_records"][0]["summary"], "f3");
    assert!(
        data["verification_records"][0].get("replace").is_none(),
        "replace is a write-time instruction, not persisted state"
    );
    land("f4", false);
    assert_eq!(
        ids(&landing.assemble(&json!({"records_landed":1})).unwrap()),
        vec!["f4", "f3"]
    );
}
...
fn replace_on_a_verify_record_drops_the_inherited_failure() {
    ...
    let record =
        &landing.assemble(&json!({"records_landed":1})).unwrap()["verification_records"][0];
    assert_eq!(
        record["status"], "failed",
        "without replace the earlier failure is inherited"
    );
    assert_eq!(
        record["summary"],
...
fn reduce_finding_keeps_its_own_task_id_instead_of_the_call_id() {
    ...
    assert_eq!(
        data["findings"][1]["canonical_task_ids"],
        json!(["TASK-2", "TASK-3"])
    );
    assert_eq!(
        data["findings"][2]["canonical_task_ids"],
        json!(["TASK-4"]),
        "an explicit canonical_task_ids wins over task_id"
    );
    assert!(
        data["findings"]
            .as_array()
            .unwrap()
            .iter()
            .all(|f| f["canonical_task_ids"] != json!(["some-reduce"]))
    );
}
...
fn unattributable_reduce_finding_lands_without_a_task_stamp() {
    ...
    let data = landing.assemble(&json!({"records_landed":1})).unwrap();
    assert!(
        data["findings"][0].get("canonical_task_ids").is_none(),
        "{}",
        data["findings"][0]
    );
...
fn skeleton_record_with_a_full_task_lands_and_assembles_under_tasks() {
    ...
    for field in ["task_id", "file_name", "depends_on", "blocks", "implements"] {
        assert_eq!(landed[field], task[field], "{field}");
    }
    for field in ["kind", "artifact_path", "min_instances"] {
        assert_eq!(
            landed["deliverable_contracts"][0][field], task["deliverable_contracts"][0][field],
            "{field}"
        );
    }
}
...
fn skeleton_stub_without_obligations_or_deliverables_is_rejected_with_the_schema() {
    ...
    assert!(
        error.contains("cannot be owned or verified"),
        "omitted arrays are the same stub: {error}"
    );
