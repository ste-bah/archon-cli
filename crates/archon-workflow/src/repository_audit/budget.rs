...
impl Limit {
    ...
    fn milliseconds(self) -> WorkflowResult<Option<u64>> {
        self.finite().map(|n| n.checked_mul(1000).filter(|v| *v > 0 && *v <= i64::MAX as u64)
            .ok_or_else(|| WorkflowError::StateCorrupt("audit duration is invalid or overflows milliseconds".into()))).transpose()
    }
}
...
fn paused(reason: &str) -> WorkflowError {
    WorkflowError::ControlPaused(format!("repository audit budget: {reason}; use an authorized operator budget extension"))
}
impl AuditBudget {
    pub fn new(policy: AuditPolicy) -> Self {
        Self { policy, spent_ms: 0, unexpected_refreshes: 0, active: None, recovered_attempts: vec![] }
    }
    ...
    pub fn begin(&mut self, id: &str, now_ms: i64, unexpected: bool) -> WorkflowResult<Option<u64>> {
        if id.is_empty() || self.active.is_some() {
            return Err(WorkflowError::StateCorrupt("audit attempt missing identity or another attempt is active".into()));
        }
        let attempt = self.policy.attempt_timeout_secs.milliseconds()?;
        let remaining = self.policy.total_time_secs.milliseconds()?.map(|total| total.saturating_sub(self.spent_ms));
        if remaining == Some(0) { return Err(paused("cumulative time exhausted")); }
        if unexpected && self.policy.unexpected_change_refreshes.finite().is_some_and(|limit| self.unexpected_refreshes >= limit) {
        ...
        if unexpected {
            self.unexpected_refreshes = self.unexpected_refreshes.checked_add(1)
                .ok_or_else(|| WorkflowError::StateCorrupt("audit refresh count overflow".into()))?;
        }
        self.active = Some(ActiveAttempt { id: id.into(), started_ms: now_ms, accounted_until_ms: now_ms, allowance_ms });
        Ok(allowance_ms)
    }
    ...
    pub fn heartbeat(&mut self, id: &str, now_ms: i64) -> WorkflowResult<()> {
        let active = self.active.as_mut().filter(|a| a.id == id)
            .ok_or_else(|| WorkflowError::StateCorrupt("audit attempt identity changed".into()))?;
        let delta = now_ms.checked_sub(active.accounted_until_ms).filter(|n| *n >= 0)
            .ok_or_else(|| WorkflowError::ControlPaused("audit clock moved backwards; recorded usage retained".into()))? as u64;
        self.spent_ms = self.spent_ms.checked_add(delta)
