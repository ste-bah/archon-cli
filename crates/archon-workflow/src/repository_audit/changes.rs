...
fn index(snapshot: &Snapshot) -> WorkflowResult<BTreeMap<String, String>> {
    if snapshot.root.join(".git").exists() { return snapshot.content_index(); }
    // Explicit non-repository snapshots have no Git object database.
    snapshot.paths.iter().map(|path| {
...
pub(super) fn between(previous: Option<&Snapshot>, current: &Snapshot) -> WorkflowResult<Vec<Value>> {
    let Some(previous) = previous else { return Ok(Vec::new()); };
    if previous.identity == current.identity { return Ok(Vec::new()); }
    let old = index(previous)?;
    let new = index(current)?;
    Ok(old.keys().chain(new.keys()).collect::<BTreeSet<_>>().into_iter()
        .filter(|path| old.get(*path) != new.get(*path))
        .map(|path| json!({"path":path,"kind":match (old.contains_key(path),new.contains_key(path)) {
