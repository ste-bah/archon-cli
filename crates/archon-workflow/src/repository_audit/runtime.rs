...
impl AuditRuntime {
    ...
    pub async fn assess(&self, snapshot: &Snapshot, paths: &[String], trigger: &str, dispatch: &dyn WorkflowAgentDispatch) -> WorkflowResult<()> {
        ...
        let future = async {
            ...
            let execution=WorkflowV2CallExecution {call:WorkflowV2HostCall{id:attempt_id.clone(),method:WorkflowV2HostMethod::Agent,write_mode:None,options},input:json!({"snapshot":snapshot.identity}),depends_on:vec![]};
