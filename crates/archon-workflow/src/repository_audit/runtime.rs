...
use super::{
    ...
    budget::{AuditBudget, AuditPolicy},
    ledger::AuditLedger,
};
use crate::*;
...
impl AuditRuntime {
    ...
    pub fn require_closed(&self, snapshot: &str) -> WorkflowResult<()> {
        ...
        if state.last_error.is_some() {
            return Err(WorkflowError::StageFailed(
                "repository audit assessment unavailable".into(),
            ));
        }
        Ok(())
    }
    pub fn seal_final(&self, snapshot: &str) -> WorkflowResult<()> {
        self.update(|state| {
            ...
            {
                return Err(WorkflowError::StageFailed(
                    "repository audit cannot seal unresolved final assessment".into(),
                ));
            }
            let report = state
                .ledger
                .history
                .last()
                .ok_or_else(|| WorkflowError::StateCorrupt("audit report missing".into()))?;
            if report
                .records
                .iter()
                .map(|r| r.declared_path.clone())
                .collect::<BTreeSet<_>>()
                != state.declared_paths
            {
                return Err(WorkflowError::StateCorrupt(
                    "audit final coverage incomplete".into(),
                ));
            }
            state.final_receipt = Some(FinalReceipt {
                generation: state.generation,
                snapshot: snapshot.into(),
                declared_paths: state.declared_paths.clone(),
    ...
    pub async fn assess(
        ...
        self.assess_with(snapshot, paths, trigger, json!({}), dispatch)
            .await
    }
    ...
    pub async fn assess_with(
        ...
        let mut state = self.state()?;
        for path in paths {
            super::contract::validate_path(path)
                .map_err(|e| WorkflowError::SpecInvalid(e.to_string()))?;
        }
        let mut added_paths = paths
            .iter()
            .filter(|path| !state.declared_paths.contains(*path))
            .cloned()
            .collect::<BTreeSet<_>>();
        ...
        if reassessments.is_empty()
            && state
                .snapshot
                .as_ref()
                .is_some_and(|s| s.identity == snapshot.identity)
            && state.ledger.history.last().is_some_and(|r| {
                r.records
                    .iter()
                    .map(|r| r.declared_path.clone())
                    .collect::<BTreeSet<_>>()
        ...
        let full = AuditContract {
            schema_version: 1,
            snapshot: snapshot.identity.clone(),
            declared_paths: state.declared_paths.iter().cloned().collect(),
        };
        ...
        let receipted = if state
            .snapshot
            .as_ref()
            .is_some_and(|previous| previous.identity != snapshot.identity)
        {
            super::receipts::read_apply_receipts(&self.store, &self.run_id)?
                .into_iter()
                .rev()
                .find(|receipt| receipt.after == snapshot.identity)
        } else {
            None
        };
        ...
        let allowance = self.update(|s| {
            ...
            for request in &mut s.ledger.reassessments {
                if reassessments
                    .iter()
                    .any(|pending| pending.action_id == request.action_id)
                {
        ...
        if let (Some(started), serde_json::Value::Object(detail)) =
            (started.as_object_mut(), detail)
        {
        ...
        let future = async {
            ...
            execution.call.options.extra.insert(
                "audit_path_timeout_secs".into(),
                json!(allowance.map(|ms| {
                    ms.div_ceil(1000)
                        .div_ceil(contract.declared_paths.len().max(1) as u64)
                        .max(1)
                })),
            );
            let result = super::landing::scope(
                ...
                dispatch.run_call(
                    ...
                    Some(snapshot.root.display().to_string()),
                    &execution,
            ...
            if result.status != WorkflowV2Status::Accepted {
                return Err(WorkflowError::StageFailed(
                    "repository audit assessor did not return accepted assessment".into(),
                ));
            }
            let landed: AuditReport =
                serde_json::from_value(result.data.get("repository_audit").cloned().ok_or_else(
                    || WorkflowError::ArtifactInvalid("missing repository audit response".into()),
                )?)?;
            contract
                .validate_report(&landed)
                .map_err(|e| WorkflowError::ArtifactInvalid(e.to_string()))?;
            let report = plan.merge(&landed);
            full.validate_report(&report)
                .map_err(|e| WorkflowError::ArtifactInvalid(e.to_string()))?;
            validate_files(snapshot, &report)?;
            let corrections = super::correction::validate(
        ...
        self.update(|s| {
            s.budget
                .finish(&attempt_id, chrono::Utc::now().timestamp_millis())?;
            match &result {
                Ok((report, corrections)) => {
                    ...
                    for correction in corrections {
                        let obligation = s
                            .ledger
                            .obligations
                            .get_mut(&correction.declared_path)
                            .ok_or_else(|| {
                                WorkflowError::StateCorrupt(
                                    "corrected audit obligation missing".into(),
                                )
                            })?;
