use archon_workflow::v2::record_landing::{RecordKind, RecordLanding};
use serde_json::json;
#[test]
...
fn verification_records_preserve_failures_and_commands() {
    ...
    assert!(
        landing
            .land(json!({"subject":"unit","status":"accepted","findings":[],"evidence":[]}))
            .is_err()
    );
...
fn multiple_landings_for_subject_preserve_earlier_findings() {
    let temp = tempfile::tempdir().unwrap();
    let records = RecordLanding::open(
        temp.path().into(),
        "id".into(),
        RecordKind::Review,
        vec!["unit".into()],
    ...
    assert_eq!(
        records.assemble(&json!({"records_landed":1})).unwrap()["findings"]
            .as_array()
            .unwrap()
            .len(),
        2
...
fn issue39_verify_pass_lands_with_a_succeeded_command_and_a_review_command_is_normalised() {
    ...
    let error = landing
        .assemble(&json!({"records_landed":2}))
        .unwrap_err()
        .to_string();
    assert!(
        error.contains("records_landed=2 but host retained 1 record(s)"),
        "{error}"
    );
    let review = RecordLanding::open(
    ...
    let command = &review.assemble(&json!({"records_landed":1})).unwrap()["verification_records"]
        [0]["commands_run"][0];
...
fn issue39_skeleton_rejects_hollow_entries_by_path() {
    ...
    assert!(
        records.assemble(&json!({"records_landed":0})).unwrap()["tasks"]
            .as_array()
            .unwrap()
            .is_empty()
    );
