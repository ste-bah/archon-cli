...
async fn advanced_floor_runs_shared_verifier_before_exact_nested_command() {
    for valid in [false, true] {
        ...
        p.timeout_secs = 15;
        let body = if valid { r#"{"cells":[{"id":"one"}]}"# } else { r#"{"cells":[]}"# };
        std::fs::write(p.project.join("data/manifest.json"), body).unwrap();
        std::fs::write(p.project.join("data/registry.json"), r#"{"records":{"one":{"payload":"data/rows.json"}}}"#).unwrap();
        std::fs::write(p.project.join("data/rows.json"), r#"[{"value":1},{"value":2}]"#).unwrap();
        ...
        std::fs::write(p.project.join("data/manifest.json"),serde_json::to_vec(&manifest).unwrap()).unwrap();
        ...
        assert_eq!(String::from_utf8_lossy(&out.checks[0].stdout).contains("nested-ran"),valid);
    }
}
...
async fn replaced_target_link_voids_result_before_reuse() {
    ...
    let mut next=c.acceptance[0].clone();next.id="AC-X-002".into();c.acceptance.push(next);
    let mut next=refs[0].clone();next.acceptance_id="AC-X-002".into();refs.push(next);
    let out=observe_commands(&p,&commit,&c,"chain",&refs,&t.path().join("evidence")).await.unwrap();
...
async fn setup_failure_preserves_operational_record_and_live_audit() {
    ...
    let raw=std::fs::read(evidence.join("observation.json")).expect("setup failure evidence missing");
...
async fn after_audit_failure_preserves_nonpassing_record() {
    ...
    let cmd=format!("test -f input && mkfifo '{}'",p.project.join("fifo").display());
    ...
    let evidence=t.path().join("evidence");
    let _=observe_commands(&p,&commit,&c,"chain",&refs,&evidence).await;
    let record:serde_json::Value=serde_json::from_slice(&std::fs::read(evidence.join("observation.json")).expect("audit failure evidence missing")).unwrap();
...
async fn each_check_records_cache_identity_and_project_mutations() {
    ...
    assert!(record["check_evidence"][0]["changed_project_paths"].as_array().unwrap().contains(&serde_json::json!("data/value")));
}
