...
fn request() -> WorkflowV2AgentRequest {
    WorkflowV2AgentRequest {
        call: WorkflowV2HostCall {
            id: "repair-fixture".into(), method: WorkflowV2HostMethod::Agent,
            write_mode: None,
            options: WorkflowV2HostOptions {
                extra: [("repository_audit_contract".into(), json!({
                    "schema_version":1,"snapshot":"sealed-1","declared_paths":["src/new.rs"]
                }))].into(), ..Default::default()
            },
        },
        role: "critic".into(), task: "Audit sealed source".into(), constraints: vec![],
...
fn invalid_result_repair_preserves_full_json_and_locates_deep_value() {
    ...
    let output = envelope(json!(["legacy/implementation.rs:123"])).to_string();
    ...
    let fault = prompt.split("Fault context:").nth(1).expect("missing local window")
        .split("Previous JSON in full:").next().unwrap();
...
fn repair_locates_escaped_unicode_value_and_json_pointer_keys() {
    let output = format!(r#"{{"padding":"{}","a/b~c":["é\\bad\"path"]}}"#, "界".repeat(6_000));
    ...
    let fault = prompt.split("Fault context:").nth(1).unwrap()
        .split("Previous JSON in full:").next().unwrap();
...
fn non_json_repair_keeps_bounded_head_fallback() {
    ...
    let prompt = WorkflowV2AgentAdapter::new().build_repair_prompt(&request(), &output,
...
fn equivalent_symbol_is_normalized_in_returned_result_before_filesystem_validation() {
    ...
    std::fs::write(temp.path().join("legacy/implementation.rs"), "fn implementation() {}").unwrap();
    ...
    let reparsed = adapter.parse_agent_output(&request, &serde_json::to_string(&result).unwrap()).unwrap();
...
fn equivalent_normalization_does_not_relax_path_or_record_contracts() {
    let adapter = WorkflowV2AgentAdapter::new();
    for path in ["../escape.rs:valid", "/tmp/escape.rs:valid", "legacy/../escape.rs:valid",
        "legacy/.git/config:valid", "legacy\\file.rs:valid", "legacy/file.rs:123",
        "legacy/file.rs:bad-name", "legacy/file.rs:foo::bar", "legacy/file.rs:",
        "legacy/file.rs:foo()", "legacy/file.rs:valid\n", "legacy/file.rs:valid:other"] {
        assert!(adapter.parse_agent_output(&request(), &envelope(json!([path])).to_string()).is_err(), "{path}");
    }
    ...
    assert!(adapter.parse_agent_output(&request(), &output.to_string()).is_err());
    for reason in ["".to_owned(), "x".repeat(2_048)] {
        let mut output = envelope(json!(["legacy/file.rs:valid"]));
        output["data"]["repository_audit"]["records"][0]["reason"] = json!(reason);
        assert!(adapter.parse_agent_output(&request(), &output.to_string()).is_err());
    }
    assert!(adapter.parse_agent_output(&request(), &envelope(json!([
        "legacy/file.rs:valid", "legacy/file.rs"
    ])).to_string()).is_err());
}
...
fn equivalent_symbol_cannot_bypass_sealed_filesystem_confinement() {
    ...
    assert!(WorkflowV2AgentAdapter::new().parse_agent_output(&request, &output).is_err());
    #[cfg(unix)] {
        ...
        let error = WorkflowV2AgentAdapter::new().parse_agent_output(&request, &output).unwrap_err();
