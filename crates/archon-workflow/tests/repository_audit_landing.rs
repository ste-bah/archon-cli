use archon_workflow::{
    repository_audit::{
        AuditContract, AuditRecord, RequiredAction, Verdict, landing::AuditLanding,
    },
    *,
};
use serde_json::json;
fn record(path: &str) -> AuditRecord {
    AuditRecord {
        ...
        reason: "Not present in sealed source".into(),
    }
}
#[test]
...
async fn compact_submission_is_completed_by_host_and_missing_paths_repair_locally() {
    ...
    let contract = AuditContract {
        schema_version: 1,
        snapshot: "sealed".into(),
        declared_paths: vec!["a".into(), "b".into()],
    };
    let landing = std::sync::Arc::new(
        AuditLanding::open(
    ...
    let mut request = v2::call_data::v2_agent_request(
        "audit",
    ...
    archon_workflow::repository_audit::landing::scope(landing.clone(), async {
        ...
        assert_eq!(
            result.data["repository_audit"]["records"]
                .as_array()
                .unwrap()
                .len(),
            2
...
fn issue49_wrong_count_refusal_names_both_counts_and_the_accepted_object() {
    ...
    let (_, landing) = landed(&temp);
    let error = landing
        .complete(&json!({"schema_version":1,"snapshot":"sealed","records_landed":3}))
        .unwrap_err()
        .to_string();
    assert!(
        error.contains("records_landed=3 but host retained 2 record(s)"),
        "{error}"
    );
    assert!(
        error.contains(r#"{"schema_version":1,"snapshot":"sealed","records_landed":2}"#),
        "{error}"
    );
    let missing = landing
        .complete(&json!({"schema_version":1,"snapshot":"sealed"}))
        .unwrap_err()
        .to_string();
    assert!(
        missing.contains("records_landed=missing but host retained 2 record(s)"),
        "{missing}"
    );
}
#[test]
fn issue49_wrong_snapshot_or_schema_refusal_names_the_field() {
    ...
    let (_, landing) = landed(&temp);
    let error = landing
        .complete(&json!({"schema_version":1,"snapshot":"other","records_landed":2}))
        .unwrap_err()
        .to_string();
    assert!(
        error.contains(r#"snapshot="other" but host snapshot is "sealed""#),
        "{error}"
    );
    assert!(
        error.contains(r#"{"schema_version":1,"snapshot":"sealed","records_landed":2}"#),
    ...
    let error = landing
        .complete(&json!({"schema_version":2,"snapshot":"sealed","records_landed":2}))
        .unwrap_err()
        .to_string();
    assert!(
        error.contains("schema_version=2 but host requires 1"),
        "{error}"
    );
    let error = landing
        .complete(&json!(["not", "an", "object"]))
        .unwrap_err()
        .to_string();
...
fn issue49_hint_ends_with_the_exact_accepted_completion_object() {
    ...
    assert!(
        hint.ends_with(r#"{"schema_version":1,"snapshot":"sealed","records_landed":2}"#),
        "{hint}"
    );
...
fn issue51_hint_names_the_carried_paths_and_lists_only_the_delta_as_remaining() {
    ...
    let contract = AuditContract {
        schema_version: 1,
        snapshot: "sealed".into(),
        declared_paths: vec!["a".into()],
    };
    let landing = AuditLanding::open(temp.path().join("delta"), temp.path().into(), contract)
        .unwrap()
        .carrying(706);
    let hint = landing.hint().unwrap();
    ...
    assert!(
        hint.contains("706 other declared path(s) keep their prior verdict"),
        "{hint}"
    );
    assert!(
        hint.ends_with(r#"{"schema_version":1,"snapshot":"sealed","records_landed":1}"#),
        "{hint}"
    );
    let (_, plain) = landed(&temp);
    assert!(
        !plain.hint().unwrap().contains("keep their prior verdict"),
