...
fn combined_scratch_uses_recorded_commit_private_data_and_relative_target() {
    ...
    let repo=temp.path().join("repo");let project=temp.path().join("project");
    std::fs::create_dir_all(&repo).unwrap();std::fs::create_dir_all(project.join("data")).unwrap();
    git(&repo,&["init","-q"]);git(&repo,&["config","user.email","fixture@example.invalid"]);git(&repo,&["config","user.name","fixture"]);
    std::fs::write(repo.join("source.txt"),"committed").unwrap();git(&repo,&["add","source.txt"]);git(&repo,&["commit","-qm","fixture"]);
    let commit=git(&repo,&["rev-parse","HEAD"]);
    std::fs::write(repo.join("source.txt"),"dirty-live").unwrap();
    std::fs::write(project.join("data/input.txt"),"live-input").unwrap();
    let policy=ScratchPolicy { repository:repo.clone(),project:project.clone(),task_root:project.join("tasks"),
        scratch_parent:temp.path().join("observations"),project_inputs:vec!["data".into()],combined:true,
        toolchain_path:"/usr/bin:/bin".into(),environment:BTreeMap::new(),cargo_seed:None,
        timeout_secs:30,output_bytes:4096,scratch_bytes:16*1024*1024 };
    std::fs::create_dir_all(&policy.task_root).unwrap();
    let mut roots=ScratchRoots::prepare(&policy,&commit).unwrap();
    assert_eq!(std::fs::read_to_string(roots.project().join("source.txt")).unwrap(),"committed");
    assert_eq!(roots.project().join("target").canonicalize().unwrap(),roots.target());
    std::fs::write(roots.project().join("data/input.txt"),"scratch-change").unwrap();
    assert_eq!(std::fs::read_to_string(project.join("data/input.txt")).unwrap(),"live-input");
    let path=roots.root().to_path_buf();roots.cleanup().unwrap();assert!(!path.exists());
    assert_eq!(std::fs::read_to_string(repo.join("source.txt")).unwrap(),"dirty-live");
...
fn unsafe_input_and_credential_environment_are_rejected_before_setup() {
    let mut p=ScratchPolicy { repository:"/repo".into(),project:"/project".into(),task_root:"/project/tasks".into(),
        scratch_parent:"/scratch".into(),project_inputs:vec!["../escape".into()],combined:false,
        toolchain_path:"/usr/bin:/bin".into(),environment:BTreeMap::new(),cargo_seed:None,
        timeout_secs:1,output_bytes:1024,scratch_bytes:1024 };
    assert!(p.validate().is_err());p.project_inputs=vec!["data".into()];
...
fn directly_selected_credential_file_is_not_exported() {
    let tmp=tempfile::tempdir().unwrap();let repo=tmp.path().join("repo");let project=tmp.path().join("project");
    std::fs::create_dir_all(&repo).unwrap();std::fs::create_dir_all(project.join("tasks")).unwrap();
    git(&repo,&["init","-q"]);git(&repo,&["config","user.email","fixture@example.invalid"]);git(&repo,&["config","user.name","fixture"]);
    std::fs::write(repo.join("source"),"x").unwrap();git(&repo,&["add","."]);git(&repo,&["commit","-qm","fixture"]);
    std::fs::write(project.join("credentials.toml"),"secret").unwrap();
    let p=ScratchPolicy {repository:repo.clone(),project:project.clone(),task_root:project.join("tasks"),scratch_parent:tmp.path().join("scratch"),project_inputs:vec!["credentials.toml".into()],combined:false,toolchain_path:"/usr/bin:/bin".into(),environment:BTreeMap::new(),cargo_seed:None,timeout_secs:1,output_bytes:1024,scratch_bytes:1024*1024};
