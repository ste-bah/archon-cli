...
fn cargo_seed_keeps_registry_metadata_without_exporting_home_credentials() {
    ...
    std::fs::write(
        seed.join("registry/index/registry-id/config.json"),
        r#"{"dl":"https://example.invalid/crates"}"#,
    )
    .unwrap();
    ...
    assert!(
        roots
            .root()
            .join("cargo-home/registry/index/registry-id/config.json")
            .is_file()
    );
