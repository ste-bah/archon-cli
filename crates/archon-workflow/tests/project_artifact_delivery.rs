...
impl WorkflowAgentDispatch for ArtifactAgent {
    ...
    async fn run_call(
        ...
        if self.code {
            std::fs::write(
                PathBuf::from(root.as_ref().unwrap()).join("input"),
                "updated code",
            )
            .unwrap();
        }
        ...
        req.project_artifacts
            .add_contract_artifact_paths(universe.unwrap(), &e.input["item"]);
        req.project_artifacts.add_artifact_requirements(&e.input);
        let mut changed = vec![json!({"path":"reports/report.md"})];
        if self.code {
            changed.push(json!({"path":"input"}));
        }
...
async fn exercise_mixed(write: bool, mixed: bool, code: bool) {
    ...
    std::fs::write(repo.join("input"), "base").unwrap();
    git(&["add", "input"]);
    ...
    let e = WorkflowV2CallExecution {
        call,
        input: json!({"source_data":[{"item_id":"report","canonical_task_ids":["TASK-001"],"target_files":if mixed {vec!["input"]}else{vec![]},"artifact_requirements":["reports/report.md"]}]}),
        depends_on: vec![],
    };
    let branches = fanout_items_for_call(&e, &v2).unwrap();
    let result = v2::write::run_write_capable_v2_fanout(
        "deliver report",
        repo.to_str(),
        e,
    ...
    if code && write {
        ...
        assert_eq!(
            git(&["rev-parse", "HEAD"]),
            before,
            "project artifacts must not be silently committed to repository"
        );
    }
    if write {
        ...
        let outcome = v2
            ...
            .find(|o| {
                o.result
                    .as_ref()
                    .is_some_and(|r| r.data["canonical_task_ids"] == json!(["TASK-001"]))
            })
            .expect("host task outcome")
            .result
            .unwrap();
        ...
        assert_ne!(
            ...
            "missing report accepted as empty repo patch"
        );
    }
}
#[tokio::test]
async fn external_project_report_is_verified_and_distinguished_from_repo_patch() {
    exercise(true).await;
}
#[tokio::test]
async fn missing_external_report_cannot_pass_via_empty_repository_patch() {
    exercise(false).await;
}

#[tokio::test]
async fn mixed_task_records_artifact_delivery_alongside_repository_patch() {
    exercise_mixed(true, true, true).await;
}
#[tokio::test]
async fn mixed_task_artifact_only_change_has_delivery_evidence() {
    exercise_mixed(true, true, false).await;
}
#[tokio::test]
async fn mixed_task_missing_artifact_still_fails() {
    exercise_mixed(false, true, true).await;
}
