...
fn valid() -> Value { json!([{"declared_path":"src/new.txt","verdict":"exists_elsewhere","equivalents":["legacy/implementation.txt"],"required_action":"wire_or_migrate","reason":"Existing implementation has the same behavior."}]) }

#[test]
fn malformed_audit_record_is_not_accepted_by_envelope_parser() {
    let error=WorkflowV2AgentAdapter::new().parse_agent_output(&request(), &envelope(json!([{"nonsense":1}]))).unwrap_err();
    assert!(error.to_string().contains("declared_path"), "{error}");
}
#[test]
fn shared_declared_path_requires_one_complete_record() {
    WorkflowV2AgentAdapter::new().parse_agent_output(&request(), &envelope(valid())).unwrap();
}
#[test]
fn incomplete_duplicate_escaping_and_wrong_snapshot_records_are_rejected() {
    let adapter=WorkflowV2AgentAdapter::new();
    for records in [json!([]), json!([valid()[0],valid()[0]]),
        ...
        assert!(adapter.parse_agent_output(&request(), &envelope(records)).is_err());
    }
    assert!(adapter.parse_agent_output(&request(), &envelope(valid()).replace("sealed-1","other")).is_err());
}
