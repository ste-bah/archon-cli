...
fn finite_allowance_above_hour_and_unlimited_have_no_hidden_cutoff() {
    let policy = AuditPolicy { attempt_timeout_secs: Limit::Finite(7200), total_time_secs: Limit::Unlimited,
    ...
    assert_eq!(budget.begin("two", 4_000_000, false).unwrap(), Some(7_200_000));
    assert_eq!(budget.spent_ms, 4_000_000);
}
#[test]
fn configured_count_and_time_pause_without_reset_on_serialization() {
    let mut budget = AuditBudget::new(AuditPolicy { attempt_timeout_secs: Limit::Finite(10),
        total_time_secs: Limit::Finite(15), unexpected_change_refreshes: Limit::Finite(1) });
    budget.begin("one", 0, true).unwrap();
    budget.finish("one", 7000).unwrap();
    let mut budget: AuditBudget = serde_json::from_value(serde_json::to_value(&budget).unwrap()).unwrap();
    assert!(budget.begin("two", 7000, true).unwrap_err().to_string().contains("paused"));
...
fn simultaneous_attempts_cannot_each_spend_remaining_budget() {
    let mut budget = AuditBudget::new(AuditPolicy { attempt_timeout_secs: Limit::Finite(10),
        total_time_secs: Limit::Finite(15), unexpected_change_refreshes: Limit::Unlimited });
...
fn an_interrupted_unlimited_attempt_does_not_restore_usage() {
    let mut budget = AuditBudget::new(AuditPolicy { attempt_timeout_secs: Limit::Unlimited,
    ...
    let mut loaded: AuditBudget = serde_json::from_value(serde_json::to_value(&budget).unwrap()).unwrap();
