...
fn transitive_consumed_artifacts_reach_consumer_without_write_ownership() {
    ...
    let call=WorkflowV2HostCall{id:"write-consumer".into(),method:WorkflowV2HostMethod::Implementation,write_mode:Some(WorkflowV2WriteMode::Worktree),options:WorkflowV2HostOptions{target_files:vec!["src/consumer.rs".into()],..Default::default()}};
    let e=WorkflowV2CallExecution{call,input:json!({"item":{"canonical_task_ids":["TASK-003"],"target_files":["src/consumer.rs"]},"_workflow_project_artifact_policy":{"project_root":"/project"}}),depends_on:vec![]};
    let req=v2::call_data::v2_agent_request("implement",Some("/repo".into()),&e,Some(&universe));
    let prompt=WorkflowV2AgentAdapter::new().build_prompt(&req);
    assert!(prompt.contains("reports/audit.md"),"transitively consumed report absent from rendered prompt");
    assert!(prompt.contains("read-only dependency context"));
    assert_eq!(req.target_files,vec!["src/consumer.rs"]);
    assert_eq!(req.input["task_universe"]["tasks"].as_array().unwrap().len(),1,"upstream contracts must not become consumer obligations");
