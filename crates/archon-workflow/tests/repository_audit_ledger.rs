    ...
    ledger
        .accept(
            AuditContract {
                schema_version: 1,
                snapshot: "one".into(),
                declared_paths: vec!["new.txt".into()],
            },
            report("one", "exists_elsewhere"),
        )
        .unwrap();
...
fn refreshed_assessment_retains_original_judgment_and_needs_applied_evidence() {
    let mut ledger = AuditLedger::default();
    let contract = |s: &str| AuditContract {
        schema_version: 1,
        snapshot: s.into(),
        declared_paths: vec!["new.txt".into()],
    };
    ledger
        .accept(contract("one"), report("one", "exists_elsewhere"))
        .unwrap();
    ledger
        .accept(contract("two"), report("two", "exists_as_declared"))
        .unwrap();
    assert!(
        !ledger.unresolved("two").unwrap().is_empty(),
        "positive text is not successful apply evidence"
    );
    ledger.record_applied("new.txt", "commit".into());
    ledger
        .accept(contract("three"), report("three", "exists_as_declared"))
        .unwrap();
