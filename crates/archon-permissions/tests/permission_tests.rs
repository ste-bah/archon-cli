...
fn find_delete_is_dangerous() {
    let (s, r, d) = no_overrides();
    assert_eq!(
        classify_command("find /tmp/cache -type f -delete", &s, &r, &d),
...
fn find_exec_rm_is_dangerous() {
    let (s, r, d) = no_overrides();
    assert_eq!(
        classify_command("find . -name '*.tmp' -exec rm -rf {} +", &s, &r, &d),
        CommandClass::Dangerous
    );
}
...
fn find_execdir_is_dangerous() {
    let (s, r, d) = no_overrides();
    assert_eq!(
        classify_command("find . -execdir touch marker {} +", &s, &r, &d),
        CommandClass::Dangerous
    );
}
...
fn find_destructive_predicates_with_attached_redirection_are_dangerous() {
    ...
    for command in [
        ...
        assert_eq!(
            classify_command(command, &s, &r, &d),
            CommandClass::Dangerous,
            "{command}"
        );
    }
...
fn ordinary_find_remains_safe() {
    let (s, r, d) = no_overrides();
    assert_eq!(
        classify_command("find . -name '*.rs' -print", &s, &r, &d),
        CommandClass::Safe
    );
}
...
fn quoted_find_delete_predicate_is_dangerous() {
    let (s, r, d) = no_overrides();
    assert_eq!(
        classify_command("find . '-delete'", &s, &r, &d),
        CommandClass::Dangerous
    );
}
