...
impl PlanStore {
    /// Resolve evidence identities against durable completion and verifier records.
    pub fn resolve_required_evidence(
        ...
        for (sequence, evidence_id) in evidence_ids.iter().enumerate() {
            ...
            } else if evidence.status == EvidenceStatus::Passed && verified(&self.db, evidence) {
...
fn untrusted(message: impl AsRef<str>) -> std::io::Error {
    std::io::Error::new(
...
fn verified(db: &cozo::DbInstance, evidence: &CompletionEvidence) -> bool {
    if evidence.provenance_record_id.trim().is_empty() || !persisted_matches(db, evidence) {
        return false;
    }
    ...
    let query = match evidence.evidence_kind {
        EvidenceKind::GateResult | EvidenceKind::ReviewFinding => {
            ...
            "?[gate_id] := *verification_gate_results{gate_id, run_id: $rid, gate_name: $name, passed: true, provenance_record_id: $provenance}, gate_id = $gid"
        }
        _ => {
            "?[gate_id] := *verification_gate_results{gate_id, run_id: $rid, passed: true, provenance_record_id: $provenance}"
        }
    };
