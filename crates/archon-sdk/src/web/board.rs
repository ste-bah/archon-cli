...
//!
//! Reaching it is a separate problem with its own rules — this module cannot
//! borrow the host session's memory handle and must not open the database
//! itself — and it lives next door in `board_store.rs`, whose doc comment says
//! why. Everything below assumes the reader it hands back.
//!
...
// A child module, not a sibling of `board`: `mod.rs` names the store as
// `board::WebBoardStore` and the election tests live in this module's test tree,
// both of which keep working only from inside here.
#[path = "board_store.rs"]
mod board_store;

pub use board_store::WebBoardStore;
