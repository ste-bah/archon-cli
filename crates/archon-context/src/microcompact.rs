...
pub fn microcompact_messages(
    ...
    // Summary replaces the removed messages, with the originating task
    ...
    match crate::compact::preserved_task(messages, split) {
        Some(task) => result.push(ContextMessage::user(&crate::compact::prepend_task_block(
            &task,
            summary_text,
        ))),
        None => result.push(ContextMessage::user(summary_text)),
    }
    // Boundary marker.
