...
use archon_pipeline::learning::confidence::{
    batch_calculate_confidence, calculate_confidence, calibrate_confidence, filter_patterns,
    rank_patterns,
};
...
mod confidence_tests {
    ...
    fn sigmoid_calibration_at_midpoint() {
        ...
        assert!(
            (cal - 0.5).abs() < 0.01,
            "sigmoid at 0.5 should be ~0.5, got {}",
            cal
        );
    }
    ...
    fn ranking_by_confidence_desc() {
        let patterns = vec![("p1", 0.3f64), ("p2", 0.9), ("p3", 0.6)];
    ...
    fn batch_confidence_calculation() {
        let inputs = vec![(0.9, 0.8, 0.7), (0.5, 0.5, 0.5)];
...
mod pattern_store_tests {
    ...
    fn ema_success_rate_update() {
        ...
        assert!(
            (updated.success_rate - expected).abs() < 1e-6,
            "expected {}, got {}",
            expected,
