...
async fn glob_caps_matches_and_counts_omitted_on_both_paths() {
    ...
    for denied_directory_names in [vec![], vec![".archon".into()]] {
        ...
        assert!(
            result.content.contains("37 matches omitted"),
            "{}",
            result.content
        );
...
async fn glob_bounds_long_paths_without_splitting_paths_or_losing_omitted_count() {
    ...
    for n in 0..200 {
        std::fs::write(dir.join(format!("{n:03}-{}.txt", "y".repeat(180))), "x").unwrap();
    }
    let ctx = ToolContext {
        working_dir: root.path().into(),
        ..Default::default()
    ...
    assert!(
        result.content.len() <= 32 * 1024 + 200,
        "{} bytes",
        result.content.len()
    );
    let shown = result
    ...
    assert!(
        result
            .content
            .contains(&format!("{} matches omitted", 200 - shown))
    );
}
