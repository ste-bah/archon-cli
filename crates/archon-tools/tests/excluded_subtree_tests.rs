...
fn fixture() -> (tempfile::TempDir, ToolContext) {
    ...
    for (path, text) in [
        ...
        (
            ".archon/workflows/old/v2/worktrees/src/stale.rs",
            "STALE needle",
        ),
    ...
    let ctx = ToolContext {
        ...
        denied_directory_names: vec![".archon".into()],
        ..Default::default()
    };
    (root, ctx)
}
...
async fn excluded_subtree_direct_read_grep_and_glob_are_refused() {
    ...
    let grep = GrepTool
        .execute(json!({"path":path,"pattern":"needle"}), &ctx)
        .await;
    let glob = GlobTool
        .execute(json!({"path":".archon","pattern":"**/*.rs"}), &ctx)
        .await;
    for result in [read, grep, glob] {
        assert!(
            result.is_error,
            "excluded subtree was admitted: {}",
            result.content
        );
...
async fn excluded_subtree_recursive_search_does_not_return_stale_source() {
    let (_root, ctx) = fixture();
    let grep = GrepTool
        .execute(json!({"pattern":"needle","output_mode":"content"}), &ctx)
        .await;
...
async fn excluded_subtree_symlink_alias_cannot_expose_stale_source() {
    ...
    let read = ReadTool
        .execute(
            json!({"file_path":"alias/workflows/old/v2/worktrees/src/stale.rs"}),
            &ctx,
        )
        .await;
    ...
    let glob = GlobTool
        .execute(json!({"pattern":"alias/**/*.rs"}), &ctx)
        .await;
    assert!(!glob.content.contains("stale.rs"), "{}", glob.content);
}
...
async fn ordinary_context_keeps_existing_access() {
    ...
    ctx.denied_directory_names.clear();
    assert!(
        !ReadTool
            .execute(
                json!({"file_path":".archon/workflows/old/v2/worktrees/src/stale.rs"}),
                &ctx
            )
            .await
            .is_error
