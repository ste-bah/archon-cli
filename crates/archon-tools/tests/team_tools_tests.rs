...
async fn team_create_tool_creates_files() {
    ...
    assert!(!result.is_error, "TeamCreate must succeed: {}", result.content);
    ...
    assert!(team_dir.join("team.json").exists(), "team.json must be created");
    assert!(team_dir.join("inbox-coder.jsonl").exists(), "coder inbox must exist");
    assert!(team_dir.join("inbox-tester.jsonl").exists(), "tester inbox must exist");
}
...
async fn send_and_read_messages() {
    ...
    let create_result = create_tool.execute(serde_json::json!({
        "name": "msg-team",
        "members": [
    ...
    let send_result = send_tool.execute(serde_json::json!({
        "team_id": team_id,
        "from": "sender",
        "to": "receiver",
    ...
    assert!(!send_result.is_error, "SendMessage must succeed: {}", send_result.content);
    ...
    assert!(!read_result.is_error, "ReadTeamMessages must succeed: {}", read_result.content);
    ...
    let read_result2 = read_tool.execute(serde_json::json!({
        "team_id": team_id,
        "role": "receiver"
    }), &ctx(tmp.path())).await;
...
async fn send_to_all_delivers_to_all_members() {
    ...
    for role in ["m1", "m2", "m3"] {
        ...
        assert_eq!(msgs.len(), 1, "role {} should have received 1 message", role);
    }
}
...
async fn team_delete_removes_files() {
    ...
    let delete_result = delete_tool.execute(serde_json::json!({
        "team_id": team_id
    }), &ctx(tmp.path())).await;
    assert!(!delete_result.is_error);
    assert!(!team_dir.exists(), "team directory must be removed after delete");
}
