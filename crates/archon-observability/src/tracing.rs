//! Structured tracing + secret redaction for archon-cli.
//!
//! Lifted from `crates/archon-tui/src/observability_tracing.rs` in
//! TASK-AGS-OBS-905 as part of the Stage 10 LIFT sequence — content is
//! verbatim save for this module-header comment. The archon-tui module
//! is now a thin re-export shim pointing at this location so existing
//! `archon_tui::observability::{init_tracing, span_*, RedactionLayer}`
//! call sites keep working unchanged during the lift.
//!
//! Hardened for Prometheus/log correctness in TASK-AGS-OBS-902; the
//! security contract below (every secret shape in `REDACTION_RE` must
//! redact) is preserved bit-for-bit across the lift.
//!
...
//! would be a catastrophic secret leak. Prior drafts of the original file
//! had that bug; this tombstone comment is preserved across the lift so
