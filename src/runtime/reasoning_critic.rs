...
fn critic_model(runtime: &CriticRuntime) -> String {
    let configured = runtime.config.model.trim();
    if configured.is_empty() {
        return runtime.model_fallback.clone();
    }
    if runtime.config.provider == "default"
        && !provider_advertises_model(runtime.provider.as_ref(), configured)
    {
        tracing::warn!(
            provider = %runtime.provider.name(),
            configured_model = configured,
            fallback_model = %runtime.model_fallback,
            "reasoning-quality critic model is not advertised by the active provider; using active session model"
        );
        return runtime.model_fallback.clone();
    }
...
fn provider_advertises_model(provider: &dyn LlmProvider, model: &str) -> bool {
    let models = provider.models();
    models.is_empty() || models.iter().any(|candidate| candidate.id == model)
}
