...
impl PendingControl {
    fn prepare(project: &Path, action: AuditAction) -> anyhow::Result<Self> {
        ...
        Ok(Self {
            ...
            prior_policy: state.budget.policy,
        })
    }
    fn confirm(self, confirmation: &str) -> anyhow::Result<()> {
        ...
        store.with_run_lock(self.action.run_id(), |store| {
            ...
            if run.generation != self.generation
                || matches!(
                    run.status,
                    archon_workflow::RunStatus::Completed | archon_workflow::RunStatus::Cancelled
                )
            {
                return Err(WorkflowError::PolicyDenied(
                    "audit confirmation has stale generation or terminal run".into(),
                ));
            }
            let mut state =
                crate::command::workflow_audit_control::read_state(store, self.action.run_id())?;
            if state.generation != self.generation
                || state.budget.policy != self.prior_policy
                || state.ledger.history.len() != self.assessment_count
            {
                return Err(WorkflowError::PolicyDenied(
                    "audit policy changed since confirmation was requested".into(),
                ));
            }
...
impl OperatorInbox {
    pub(super) fn handle_input(
        ...
        if !input.starts_with("/workflow audit ") {
            return None;
        }
    ...
    fn dispatch(&mut self, project: &Path, input: &str) -> anyhow::Result<String> {
        if input.starts_with("/workflow audit confirm ") {
            let pending = self
                .pending
                .take()
                .ok_or_else(|| anyhow::anyhow!("no pending audit confirmation"))?;
        ...
        let args = [
            vec!["archon".to_string(), "workflow".to_string()],
            parsed.raw_args,
        ]
        .concat();
        let cli = crate::cli_args::Cli::try_parse_from(args)?;
        let Some(crate::cli_args::Commands::Workflow {
            action: crate::cli_args::WorkflowAction::Audit { action },
        }) = cli.command
        else {
        ...
        if let AuditAction::Status { run_id } = &action {
            ...
            return Ok(serde_json::to_string_pretty(
                &crate::command::workflow_audit_control::read_state(&store, run_id)?.status()?,
            )?);
        }
...
mod tests {
    ...
    fn fixture() -> (tempfile::TempDir, AuditRuntime) {
        ...
        audit
            .update(|s| {
                s.budget.spent_ms = 500;
                s.budget.unexpected_refreshes = 2;
                Ok(())
            })
            .unwrap();
        (temp, audit)
    ...
    fn repository_audit_confirmed_extension_preserves_consumption() {
        ...
        assert_eq!(
            state.budget.policy.unexpected_change_refreshes,
            Limit::Finite(5)
        );
    ...
    fn repository_audit_human_input_dispatch_requires_exact_confirmation() {
        ...
        let request = format!(
            "/workflow audit extend-budget {} --extra-refreshes 2 --reason \"larger repository\"",
            audit.run_id
        );
        let preview = inbox
            .handle_input(temp.path(), &request)
            .expect("operator input must be intercepted")
            .unwrap();
        assert_eq!(
            audit
                .state()
        ...
        let confirmation = preview
            .lines()
            .find(|line| line.starts_with("/workflow audit confirm "))
            .unwrap()
            .to_string();
        inbox
            .handle_input(temp.path(), &confirmation)
            .unwrap()
            .unwrap();
        assert_eq!(
            audit
    ...
    fn repository_audit_waiver_preserves_judgment_and_expires_on_new_snapshot() {
        let (temp, audit) = fixture();
        open_finding(&audit);
        let pending = PendingControl::prepare(
            temp.path(),
            AuditAction::Waive {
                run_id: audit.run_id.clone(),
                finding: "file.txt".into(),
                snapshot: "one".into(),
        ...
        let state = audit.state().unwrap();
        assert_eq!(
            state.ledger.history[0].records[0].required_action,
            archon_workflow::repository_audit::RequiredAction::WireOrMigrate
        );
        assert!(
            state.ledger.obligations["file.txt"]
                .resolved_snapshot
                .is_none()
        );
        ...
        audit
            .update(|s| {
                let mut report = s.ledger.history[0].clone();
                report.snapshot = "two".into();
                s.snapshot.as_mut().unwrap().identity = "two".into();
                s.ledger.accept(
                    archon_workflow::repository_audit::AuditContract {
                        schema_version: 1,
                        snapshot: "two".into(),
                        declared_paths: vec!["file.txt".into()],
                    },
                    report,
                )
            })
    ...
    fn repository_audit_reassessment_request_never_grants_acceptance_or_budget() {
        ...
        let pending = PendingControl::prepare(
            ...
            AuditAction::Reassess {
                ...
                finding: "file.txt".into(),
                snapshot: "one".into(),
                reason: "inspect the alternate entry point".into(),
            },
        )
        .unwrap();
        let confirmation = format!("/workflow audit confirm {}", pending.id);
