...
impl RequestBroker {
        ...
        let task = archon_observability::spawn_named("audit-operator-requests", async move {
            while let Ok((mut stream, _)) = listener.accept().await {
                let result = tokio::time::timeout(std::time::Duration::from_secs(2), async {
                    ...
                    sender
                        .try_send(action)
                        .map_err(|_| anyhow::anyhow!("operator request queue is full"))?;
                    Ok::<_, anyhow::Error>(())
                })
                .await;
                let reply = if matches!(result, Ok(Ok(()))) {
                    b"queued".as_slice()
                } else {
                    b"rejected".as_slice()
                };
    ...
    pub(super) fn try_receive(&mut self) -> Option<AuditAction> {
        self.receiver.try_recv().ok()
    }
    #[cfg(test)]
    pub(super) async fn receive(&mut self) -> Option<AuditAction> {
...
mod tests {
    ...
    async fn repository_audit_cli_request_reaches_host_without_mutating_state() {
        ...
        let action = AuditAction::ExtendBudget {
            ...
            extra_seconds: None,
            reason: "more source".into(),
        };
        crate::command::workflow_audit_control::handle_cli(temp.path(), &action)
            .await
            .unwrap();
        let queued = tokio::time::timeout(std::time::Duration::from_secs(2), broker.receive())
            .await
            .unwrap()
            .unwrap();
