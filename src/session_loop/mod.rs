...
use session_shutdown::{combine_shutdown_results, finish_session};
...
pub(crate) fn run_session_loop(
    ...
    Box::pin(async move {
        ...
        *cancel_handle_slot.lock().unwrap() = Some(Arc::clone(&adapter));
        ...
        let decomposition_result = cmd_ctx.fixed_decomposition_owner.cancel_and_wait().await;
        let session_result =
            finish_session(&agent_def, &agent, &agent_dispatcher, sandbox_audit_drain).await;
        let shutdown_result = combine_shutdown_results(decomposition_result, session_result);
