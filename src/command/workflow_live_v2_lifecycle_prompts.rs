...
pub(crate) const VERIFICATION_PLAN_TASK: &str = "Plan focused verification for newly implemented or remediated canonical task IDs before dependents can unblock. Return one verification item per exact command/check whenever possible. Each item must include item_id, canonical_task_ids, focused_verification, expected_evidence, optional artifact_requirements, and source_item_id. Diff-scope checks must use the source branch write_coordination_scope declared_target_files and changed_files, never git status of the shared repository tree. Artifact checks must enumerate exact artifact fields or paths; never fail on vague requirements such as all required artifact path fields or all expected artifacts unless the item lists them explicitly. Check project artifacts relative to projectArtifactRoot when present, and repository source files relative to targetRepositoryRoot. When provider/API credentials matter, include provider_env_requirements with exact env key names and require redacted provider_env_proof; never include values. Evidence is accepted when at least one intended target passes and no intended target fails; duplicate cargo workspace/lib/bin harness pass sections are valid as one canonical pass.";
...
pub(crate) const VERIFICATION_WAVE_TASK: &str = "Run focused verification only. The input names target_repository_root and project_artifact_root explicitly; repository source paths resolve against target_repository_root, while .archon/... project artifact paths resolve against project_artifact_root, never the repository working directory. For diff-scope checks, validate write_coordination_scope declared_target_files against changed_files; never use shared-tree git status as branch ownership evidence. Return structured status, item_id, canonical_task_ids, focused_verification executed, commands run with exit_code and output_summary, matched test/check names, pass/fail count, artifacts checked, source_item_id, verification_failure_class when failed, and residual gaps. Accept duplicate cargo workspace/lib/bin harness pass sections as one canonical pass when at least one intended target passes and no intended target fails. Do not modify files.";

pub(crate) const VERIFICATION_FAILURE_TRIAGE_TASK: &str = "Classify failed focused verification outcomes. Return actionable implementation failures in implementation_failures as items requiring write remediation, retryable verification-shape issues in retry_items only for checks that actually require rerun, and accepted sibling-covered failures in superseded_items. implementation_failures, retry_items, and superseded_items must include item_id, source_item_id, canonical_task_ids, source_residual_gap_ids, failed_predicate, focused_verification or recommended_retry, and expected_evidence. Preserve every source residual gap ID and its failed predicate; dropping either is a contract error. Allowed classifications include actionable_implementation_failure, retryable_verification_shape_issue, retry_resolved_by_sibling_evidence, and retry_resolved_verification_execution_issue. Return terminal external/runtime/safety blockers with concrete evidence. Do not mark tasks complete.";
...
pub(crate) const POST_REMEDIATION_VERIFICATION_PLAN_TASK: &str = "Plan focused verification after write remediation. Return one item per exact command/check with item_id, canonical_task_ids, focused_verification, expected_evidence, optional artifact_requirements, and source_item_id. Diff-scope checks must use write_coordination_scope declared_target_files and changed_files, never shared-tree git status. Include provider_env_requirements for provider-sensitive checks and require redacted provider_env_proof only.";
...
pub(crate) const POST_REMEDIATION_VERIFICATION_WAVE_TASK: &str = "Run focused post-remediation verification only. The input names target_repository_root and project_artifact_root explicitly; resolve .archon/... paths against project_artifact_root, never the repository working directory. For diff-scope checks, validate write_coordination_scope declared_target_files against changed_files; never use shared-tree git status. Return structured status, item_id, canonical_task_ids, commands run with exit_code and output_summary, matched checks, pass/fail count, artifacts checked, source_item_id, verification_failure_class when failed, and residual gaps. Do not modify files.";

pub(crate) const VERIFICATION_REPAIR_PLAN_TASK: &str = "Repair failed focused verification shape into a concrete retry plan only when the issue is missing/malformed verification evidence, target selection, or retryable command shape. Every retry item must preserve source_residual_gap_ids and failed_predicate from the failed outcome and verify that exact predicate. If a predicate is false about the execution environment, return route: predicate_unsatisfiable_as_written with re_authored_items grounded in runtime-provided evidence and preserving the original source gap ID; do not blindly retry it. Diff-scope checks must use write_coordination_scope, never shared-tree git status. For a genuine missing concrete artifact or product-code failure, return route: write_remediation with the affected source outcome IDs instead of another retry item. Do not use verification repair for product-code or artifact-contract failures; those require write remediation.";

pub(crate) const VERIFICATION_REPAIR_SHAPE_REPAIR_TASK: &str = "Repair malformed focused verification retry output into canonical items. Return items with item_id, canonical_task_ids, focused_verification, expected_evidence or artifact_requirements, source_item_id, and provider_env_requirements when provider credentials are required. The host stamps source_residual_gap_ids and failed_predicate from the matched failed outcome; do not invent or replace gap identity. Artifact checks must enumerate exact fields or paths and must not use vague all required artifact path fields or all expected artifacts wording. Project artifact checks must name projectArtifactRoot-relative paths when possible.";

pub(crate) const RETRY_VERIFICATION_WAVE_TASK: &str = "Run repaired focused verification only. The input names target_repository_root and project_artifact_root explicitly; repository source paths resolve against target_repository_root, while .archon/... project artifact paths resolve against project_artifact_root, never the repository working directory. Diff-scope retries must validate write_coordination_scope and never shared-tree git status. Return structured status, item_id, canonical_task_ids, focused_verification executed, commands run with exit_code and output_summary, matched test/check names, pass/fail count, artifacts checked, source_item_id, verification_failure_class when failed, and residual gaps. Do not modify files.";
