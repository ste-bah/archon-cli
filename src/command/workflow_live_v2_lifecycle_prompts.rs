...
pub(crate) const VERIFICATION_FAILURE_TRIAGE_TASK: &str = "Classify failed focused verification outcomes. Return actionable implementation failures as items requiring write remediation, retryable verification-shape issues as retry_items only for checks that actually require rerun, and accepted sibling-covered failures as superseded_items. retry_items and superseded_items must include item_id, source_item_id, canonical_task_ids, source_residual_gap_ids, failed_predicate, focused_verification or recommended_retry, and expected_evidence. Preserve every source residual gap ID and its failed predicate; dropping either is a contract error. Allowed classifications include retryable_verification_shape_issue, retry_resolved_by_sibling_evidence, and retry_resolved_verification_execution_issue. Return terminal external/runtime/safety blockers with concrete evidence. Do not mark tasks complete.";
