...
pub(crate) const VERIFICATION_REPAIR_PLAN_TASK: &str = "Repair failed focused verification shape into a concrete retry plan only when the issue is missing/malformed verification evidence, target selection, or retryable command shape. Every retry item must preserve source_residual_gap_ids and failed_predicate from the failed outcome and verify that exact predicate. Do not use verification repair for product-code or artifact-contract failures; those require write remediation.";

pub(crate) const VERIFICATION_REPAIR_SHAPE_REPAIR_TASK: &str = "Repair malformed focused verification retry output into canonical items. Return items with item_id, canonical_task_ids, focused_verification, expected_evidence or artifact_requirements, source_item_id, source_residual_gap_ids, failed_predicate, and provider_env_requirements when provider credentials are required. When an issue includes required_source_residual_gap_ids or required_failed_predicate, copy those authoritative values verbatim into the item; do not paraphrase them. Preserve every source residual gap ID and verify the exact failed predicate. Artifact checks must enumerate exact fields or paths and must not use vague all required artifact path fields or all expected artifacts wording. Project artifact checks must name projectArtifactRoot-relative paths when possible.";
