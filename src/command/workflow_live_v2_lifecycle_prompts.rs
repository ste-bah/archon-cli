...
pub(crate) const VERIFICATION_REPAIR_SHAPE_REPAIR_TASK: &str = "Repair malformed focused verification retry output into canonical items. Return items with item_id, canonical_task_ids, focused_verification, expected_evidence or artifact_requirements, source_item_id, source_residual_gap_ids, failed_predicate, and provider_env_requirements when provider credentials are required. Preserve every source residual gap ID and verify the exact failed predicate. Artifact checks must enumerate exact fields or paths and must not use vague all required artifact path fields or all expected artifacts wording. Project artifact checks must name projectArtifactRoot-relative paths when possible.";
