...
pub(crate) const ARTIFACT_REQUIREMENTS_DISCOVERY_TASK: &str = "Investigate artifact requirements from PRD/TASK files. Return repaired items with artifact_requirements, using an empty array only when no artifact is required and that no-op is evidenced.";
...
pub(crate) const REMEDIATION_OUTCOME_REPAIR_TASK: &str = "Repair unresolved remediation outcomes into a follow-up remediation inventory, preserving original source-owned target files, canonical task IDs, verification, and artifact requirements. Do not infer target files from workflow scripts, branch result JSON, or artifact paths. Use target_files only for repo-owned implementation file paths; artifact-only remediation must set target_files to [] and put concrete requirements in artifact_requirements. Never place instructions, evidence guidance, or prose in target_files; fix instructions belong in required_fix.";
...
pub(crate) const REVIEW_REMEDIATION_INVENTORY_REPAIR_TASK: &str = "Repair review remediation inventory shape before write fanout. Return full replacement items only. Use canonical taskUniverse IDs in canonical_task_ids/dependency_ids. Include target_files only for repo-owned source edits; use [] for artifact-only remediation with concrete artifact_requirements. Preserve failure context, required_fix, focused_verification, and artifact requirements. Do not invent synthetic canonical task IDs.";
