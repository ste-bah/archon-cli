...
pub(crate) const REMEDIATION_OUTCOME_REPAIR_TASK: &str = "Repair unresolved remediation outcomes into a follow-up remediation inventory, preserving original source-owned target files, canonical task IDs, verification, and artifact requirements. Do not infer target files from workflow scripts, branch result JSON, or artifact paths.";
