...
async fn a_criterion_in_outcome_language_makes_its_findings_repairable() {
    let temp = tempfile::tempdir().unwrap();
    let (tasks, prd) = seed_with_criterion(&temp, "output is valid");
    let error = prepare_acceptance_freeze(
        ...
        GateMode::Observe,
        Arc::new(JudgeClient {
            result: Err("judge must not run".into()),
        }),
    )
    .await
    .unwrap_err();
    assert!(CandidateRejected::caused(&error), "{error:#}");
    assert!(!format!("{error:#}").contains("judge must not run"));
}
...
async fn a_criterion_prescribing_the_check_shape_keeps_its_findings_inherited() {
    ...
    assert!(
        scopes
            .iter()
            .all(|scope| *scope == archon_workflow::RemediationScope::InheritedPredecessor),
        "{scopes:?}"
    );
}
