...
mod tests {
    ...
    fn write_tradingview_cli(
        ...
        let mut bars = (0..bar_count)
            .map(|index| {
                ...
                json!({
                    "time": 1_704_153_600_i64 + index as i64 * 86_400,
                    "open": open,
                    "high": open + 2.0,
                    "low": open - 1.0,
                    "close": open + 1.0,
        ...
        std::fs::write(
            &cli,
            format!("console.log(JSON.stringify({response}));"),
        )
        .unwrap();
    }
    ...
    fn fetch_native_tradingview_stores_complete_artifact_contract() {
        ...
        .unwrap();
        let dataset_root = temp.path().join(
            ".archon/trading-lab/data/datasets/tradingview-ES-1D-raw/20240102-tv_native_1D_100",
        );
        assert!(text.contains("\"can_fetch\": true"));
        assert!(text.contains("\"production_eligible\": true"));
