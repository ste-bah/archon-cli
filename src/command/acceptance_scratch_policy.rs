...
pub(crate) fn record_final_source(
    store:&archon_workflow::WorkflowStore,run_id:&str,binding:&serde_json::Value,
)->WorkflowResult<serde_json::Value> {
    let repo=binding.get("policy").and_then(|p|p.get("repository")).and_then(|r|r.as_str())
        .ok_or_else(||WorkflowError::StateCorrupt("native policy has no source repository".into()))?;
    let repo=Path::new(repo).canonicalize().map_err(|source|WorkflowError::Io {path:repo.into(),source})?;
    let path=store.run_dir(run_id).join("observer/source-revision.json");
    let record=if path.exists() {
        let value:serde_json::Value=serde_json::from_slice(&std::fs::read(&path).map_err(|source|WorkflowError::Io {path:path.clone(),source})?)?;
        if value["repository"].as_str()!=repo.to_str() || value["run_id"].as_str()!=Some(run_id) {
            return Err(WorkflowError::StateCorrupt("native final source record identity differs".into()));
        }
        value
    } else {
        let output=std::process::Command::new("git").arg("-C").arg(&repo).args(["rev-parse","HEAD"]).output()
            .map_err(|source|WorkflowError::Io {path:repo.clone(),source})?;
        if !output.status.success() {return Err(WorkflowError::StateCorrupt("cannot record final implementation commit".into()));}
        let commit=String::from_utf8_lossy(&output.stdout).trim().to_string();
        if commit.len()!=40 || !commit.bytes().all(|c|c.is_ascii_hexdigit()) {return Err(WorkflowError::StateCorrupt("invalid final source commit".into()));}
        let value=serde_json::json!({"run_id":run_id,"repository":repo,"commit":commit});
        store.write_run_json(run_id,"observer/source-revision.json",&value)?;value
    };
    let mut updated=binding.clone();updated["source_commit"]=record["commit"].clone();Ok(updated)
}
