...
async fn repository_audit_open_obligation_blocks_cached_write_credit() {
    ...
    let record = WorkflowV2CallRecord::new(
        ...
        1,
        "input".into(),
        WorkflowV2Result::accepted("old acceptance"),
        vec![],
    );
    assert!(
        host.mark_reused(&record, None).await.is_err(),
        "cached acceptance bypassed open audit finding"
    );
}
