...
const HARNESS_PLANNER_RULES: &str = concat!(
    ...
    "- Every executable w.* host call must pass a non-empty stable string id as its first argument. Prefer literal ids. Inside bounded JavaScript loops, deterministic ids with a literal prefix are allowed, for example `\"implementation-wave-\" + waveIndex`; do not use opaque/random ids.\n",
