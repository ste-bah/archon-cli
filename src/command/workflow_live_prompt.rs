...
const HARNESS_PLANNER_RULES: &str = concat!(
    ...
    "- Every executable w.* host call must pass a non-empty stable literal string id as its first argument. Do not compute host-call ids at runtime; stable ids are required for approval, visibility, restart, and resume cache keys.\n",
    ...
    "- Broad migrations use inventory variables -> implementation fanout with itemKind: \"implementation\" and targetFilesFromItem: true or explicit targetFiles -> focused verification/review -> remediation/fix loops as needed -> finalReport/qualityGate.\n",
    ...
    "- If the user says to fix every issue found before continuing, every adversarial review must feed a remediation inventory and fix stage before the next qualityGate or dependent workstream.\n",
    "- Every w.fanout or w.parallel that iterates work must pass the actual typed JavaScript array as its second argument, for example `w.fanout(\"id\", inventory.items, { ... })` or `w.parallel(\"id\", items, { ... })`. Do not rely on prose from a prior call as the source.\n",
