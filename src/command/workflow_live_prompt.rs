...
const PLANNER_RULES: &str = concat!(
    ...
    "- When the task, PRD, or task files name concrete required output artifacts or deliverable files, the final quality gate MUST include `required_artifacts: [...]` with those exact paths. The workflow runtime will fail the gate if any required artifact is missing; do not omit this for project artifacts outside the repository.\n",
