...
async fn repository_audit_clean_dispatch_is_not_a_final_assessment_receipt() {
    ...
    audit.update(|state| {
        state.snapshot = Some(Snapshot{identity:"dispatch".into(),root:temp.path().into(),paths:vec![]});
        state.ledger.accept(AuditContract{schema_version:1,snapshot:"dispatch".into(),declared_paths:vec![]},
            AuditReport{schema_version:1,snapshot:"dispatch".into(),records:vec![]})
    }).unwrap();
    finalize_summary(&store,&run.id,WorkflowRunKind::FixedOrSavedScript,None,
        &summary(WorkflowV2Status::Accepted),&v2,None,None).await.unwrap();
    assert_eq!(store.load_state(&run.id).unwrap().status,RunStatus::Failed,
