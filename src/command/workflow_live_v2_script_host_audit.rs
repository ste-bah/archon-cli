...
impl WorkflowScriptHost {
    ...
    pub(super) fn audit_cache_eligible(
        &self,
        ...
        if record.call.options.target_files_from_item {
            return Ok(false);
        }
    ...
    pub(super) async fn refresh_audit_for_cache(
        ...
        if record.call.write_mode.is_none() {
            return self.audit_cache_eligible(record);
        }
        if record.call.options.target_files_from_item {
            return Ok(false);
        }
            ...
            if let Some(root) = &self.runner.runtime.target_repository_root {
                ...
                audit
                    .assess(
                        ...
                        &all,
                        "cache",
                        &AuditDispatch(self.runner.client.for_audit()),
                    )
                    .await?;
            }
        }
