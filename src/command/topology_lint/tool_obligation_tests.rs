...
fn mcp_obligation_body_candidate_and_set_gate_reject_missing_declarations() {
    ...
    for mode in [GateMode::Observe, GateMode::Enforce] {
        let candidate =
            evaluate_task_file_candidate(dir.path(), &path, raw.as_bytes(), mode).unwrap();
        assert!(
            candidate
                .findings
                .iter()
                .any(|f| f.text.contains("required_tools")
                    && f.remediation_scope == archon_workflow::RemediationScope::Body),
            "{:?}",
            candidate.findings
        );
        let set = evaluate_lint(
            dir.path(),
            &LintSource::Tasks(path.parent().unwrap().into()),
            mode,
        )
        .unwrap();
        assert!(
...
fn mcp_obligation_wrong_server_or_ambient_runner_cannot_satisfy_contract() {
    for tools in [
        "[Bash]",
        "[mcp__other__fetch_records]",
        "[mcp__sample__erase_records]",
    ] {
        let (dir, path, raw) = fixture("sample-mcp-native-ingest", tools);
        let gate =
            evaluate_task_file_candidate(dir.path(), &path, raw.as_bytes(), GateMode::Enforce)
                .unwrap();
        assert!(
            gate.findings
                .iter()
...
fn mcp_obligation_repaired_task_reaches_actual_stage_allowlist() {
    ...
    let gate =
        evaluate_task_file_candidate(dir.path(), &path, raw.as_bytes(), GateMode::Enforce).unwrap();
    assert!(
        !gate
            .findings
            .iter()
    ...
    let request = archon_workflow::StageRunRequest {
        run_id: "test".into(),
        stage_id: "write".into(),
        stage_kind: archon_workflow::StageKind::Implementation,
        agent: None,
        task: "Implement".into(),
        attempt: 1,
        provider_tier: archon_workflow::ProviderTier::Coder,
        depends_on: vec![],
        input: serde_json::json!({"project_artifact_root":dir.path(), "item":item}),
    };
    let tools = crate::command::workflow_live::workflow_live_runner::allowed_tools(&request);
    assert!(
        tools.contains(&"mcp__sample__fetch_records".into()),
        "{tools:?}"
    );
...
fn mcp_obligation_exact_focused_call_must_be_declared_even_with_another_grant() {
    ...
    assert!(
        gate.findings
            .iter()
            .any(|f| f.text.contains("fetch_records") && f.text.contains("required_tools")),
        "{:?}",
        gate.findings
    );
}
...
fn issue40_undeclared_focused_invocation_lists_the_servers_permitted_tools() {
    let defects = issue40_defects(
        "[mcp__srvname__tool_check]",
        "- `mcp__srvname__tool_other` must run",
    );
    assert_eq!(defects.len(), 1, "{defects:?}");
    assert!(
        defects[0].contains("focused MCP call 'mcp__srvname__tool_other'"),
        "{defects:?}"
    );
    assert!(
        defects[0].contains(
            "permitted for that server: mcp__srvname__tool_check, mcp__srvname__tool_other"
        ),
        "{defects:?}"
    );
}
...
fn issue40_mcp_action_form_needs_a_name_to_become_an_obligation() {
    let defects = issue40_defects(
        "[mcp__srvname__tool_check]",
        "- the `mcp_action:` prefix alone names nothing",
    );
    assert!(defects.is_empty(), "{defects:?}");
    let defects = issue40_defects(
        "[mcp__srvname__tool_check]",
        "- `mcp_action:tool_other` must run",
    );
    assert_eq!(defects.len(), 1, "{defects:?}");
    assert!(
        defects[0].contains("focused MCP call 'mcp_action:tool_other'"),
        "{defects:?}"
    );
}
...
fn mcp_obligation_validate_local_task_and_frozen_chain() {
    ...
    let request = archon_workflow::StageRunRequest {
        run_id: "local-probe".into(),
        stage_id: "write".into(),
        stage_kind: archon_workflow::StageKind::Implementation,
        agent: None,
        task: "Inspect binding".into(),
        attempt: 1,
        provider_tier: archon_workflow::ProviderTier::Coder,
        depends_on: vec![],
        input: serde_json::json!({"project_artifact_root":root, "item":item}),
    };
    let tools = crate::command::workflow_live::workflow_live_runner::allowed_tools(&request);
    for name in &expected {
        assert!(tools.contains(name), "missing {name}: {tools:?}");
    }
    assert_eq!(
        tools
            .iter()
    ...
    println!(
        "Frozen chain valid; {} task-declared MCP tools reach stage allowlist; no MCP invoked",
        expected.len()
    );
}
