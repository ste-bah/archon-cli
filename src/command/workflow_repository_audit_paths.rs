...
pub(super) fn repository_paths(
    ...
    let Some(root) = root else { return Ok(repository.into_iter().collect()); };
    Ok(archon_workflow::repository_audit::ignored::repository_deliverables(
        Path::new(root), repository.into_iter().collect()))
}
...
mod tests {
    ...
    fn repository(temp: &std::path::Path) -> std::path::PathBuf {
        ...
        for args in [
            vec!["init", "-q"], vec!["add", "-f", "src/y.rs", "docs/tracked.md", ".gitignore"],
            vec!["-c", "user.name=fixture", "-c", "user.email=fixture@example.invalid", "commit", "-qm", "base"],
        ] {
    ...
    fn declared(repo: &std::path::Path, paths: &[&str]) -> BTreeSet<String> {
        paths.iter().map(|path| repo.join(path).display().to_string()).collect()
    }
    ...
    fn a_gitignored_declaration_is_not_a_repository_deliverable() {
        ...
        let paths = repository_paths(
            declared(&repo, &["docs/x.md", "src/y.rs", "docs/tracked.md"]), Some(&root), &store,
        ).unwrap();
        assert_eq!(paths, vec!["docs/tracked.md".to_string(), "src/y.rs".to_string()]);
    }
    ...
    fn without_a_repository_root_the_declaration_set_is_unchanged() {
        ...
        let repo = repository(temp.path());
        let store = WorkflowV2ResultStore::new(temp.path().join("v2"));
        let paths = repository_paths(declared(&repo, &["docs/x.md", "src/y.rs"]), None, &store).unwrap();
