...
fn render_status(config: &archon_core::sandbox::SandboxConfig, verbose: bool) -> Result<String> {
    ...
    if verbose {
        ...
        output.push_str(
            "Execution: docker can route Bash when selected; ssh and openshell remain detect-only\n",
        );
    }
    Ok(output)
}
