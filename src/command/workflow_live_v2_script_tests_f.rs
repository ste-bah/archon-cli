...
"#;
    ...
    assert!(
        error
            .to_string()
            .contains("literal repo-relative file paths"),
        "{error}"
    );
}
...
async function workflow(w) {
  ...
  const reviewTasks = [
    { item_id: "r1", canonical_task_ids: ["TASK-EX-001"], task: "review 001" },
    { item_id: "r2", canonical_task_ids: ["TASK-EX-002"], task: "review 002" },
  ];
  await w.parallel("adversarial-review-map", reviewTasks, {
  ...
  await w.reduce("adversarial-review-reduce", { findings: [] }, {
    tier: "critic",
...
"#;
    ...
    assert!(
        !error.contains("NO write coverage")
            || !error
                .split("NO write coverage")
                .nth(1)
                .unwrap_or("")
                .contains("TASK-EX-001"),
        "{error}"
    );
