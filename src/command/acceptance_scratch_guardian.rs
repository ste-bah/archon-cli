...
use std::{path::PathBuf,sync::{Arc,atomic::{AtomicBool,Ordering}},io::{BufRead,Read}};
use archon_workflow::{WorkflowError,WorkflowResult};
use archon_workflow::acceptance_scratch::{ScratchPolicy,ObservationResult,observe_commands_cancellable};
use archon_workflow::acceptance_world::{AcceptanceCommandKind,FrozenCommandRef};
use archon_workflow::task_set_contract::{AcceptanceContract,AcceptancePin,AcceptanceCheck,content_digest,validate_acceptance_bundle,ACCEPTANCE_CONTRACT_FILE};
...
pub(crate) fn validate(request:&Request)->WorkflowResult<(AcceptanceContract,String,Vec<FrozenCommandRef>)> {
    ...
    let refs=contract.acceptance.iter().chain(&contract.supplementary).filter_map(|entry| {
        let (kind,command)=match &entry.check {
            AcceptanceCheck::Command{command,..}=>(AcceptanceCommandKind::Command,command.as_str()),
            AcceptanceCheck::Floor{contract} =>(AcceptanceCommandKind::NestedVerifier,contract.typed_verifier_command.as_deref()?),
        };
        Some(FrozenCommandRef{acceptance_id:entry.id.clone(),kind,chain_digest:digest.clone(),command_digest:content_digest(command.as_bytes())})
    }).collect();
    Ok((contract,digest,refs))
}
pub(crate) async fn entry()->anyhow::Result<bool> {
    if std::env::args().nth(1).as_deref()!=Some(FLAG) {return Ok(false);}
    serve().await.map_err(anyhow::Error::new)?;Ok(true)
}
async fn serve()->WorkflowResult<()> {
    let mut line=String::new();
    let stdin=std::io::stdin();let mut reader=std::io::BufReader::new(stdin);
    reader.by_ref().take(4*1024*1024).read_line(&mut line).map_err(|e|WorkflowError::SpecInvalid(e.to_string()))?;
    if !line.ends_with('\n') {return Err(WorkflowError::SpecInvalid("native guardian request exceeded limit".into()));}
    ...
    if !result.teardown_verified || !result.live_roots_unchanged {
        return Err(WorkflowError::ArtifactInvalid("native observation void: live-root audit or teardown failed; inspect scratch evidence".into()));
    }
...
pub(crate) async fn launch(request:Request)->WorkflowResult<ObservationResult> {
    ...
    validate(&request)?;
    let mut command=tokio::process::Command::new(std::env::current_exe().map_err(|e|WorkflowError::SpecInvalid(e.to_string()))?);
    #[cfg(not(test))] command.arg(FLAG);
    #[cfg(test)] command.args(["--exact","command::acceptance_scratch_guardian::tests::guardian_entry","--ignored","--nocapture"]);
    command.env_clear().envs([("PATH","/usr/bin:/bin:/usr/sbin:/sbin")])
        .stdin(std::process::Stdio::piped()).stdout(std::process::Stdio::null()).stderr(std::process::Stdio::null());
    let mut child=command.spawn().map_err(|e|WorkflowError::SpecInvalid(e.to_string()))?;
    let mut pipe=child.stdin.take().unwrap();
    let mut bytes=serde_json::to_vec(&request)?;bytes.push(b'\n');
    pipe.write_all(&bytes).await.map_err(|e|WorkflowError::SpecInvalid(e.to_string()))?;
    let status=child.wait().await.map_err(|e|WorkflowError::SpecInvalid(e.to_string()))?;
    drop(pipe);
