...
/// JS `splitFocusedVerificationItems`.
pub(super) fn split_focused_verification_items(
    ...
    for raw in items {
        ...
        .any(|key| item.get(*key).is_some())
            || item.get("split_verification_items") == Some(&Value::Bool(false))
            || item.get("repair_type").and_then(Value::as_str)
                == Some("verification_evidence_shape");
        if keep_whole {
            split.push(item);
            continue;
        }
        let checks = strings_of(item.get("focused_verification"));
        if checks.len() <= 1 {
            split.push(item);
            continue;
        }
        ...
        for (index, check) in checks.iter().enumerate() {
            let mut clone = item.as_object().cloned().unwrap_or_default();
            let split_id = format!("{base_id}-check-{}", index + 1);
            clone.insert("item_id".to_string(), Value::String(split_id.clone()));
            clone.insert("id".to_string(), Value::String(split_id));
            clone.insert(
...
pub(super) fn has_concrete_evidence(outcome: &Value) -> bool {
...
pub(super) fn outcome_status(outcome: &Value) -> Option<&str> {
    outcome.get("status").and_then(Value::as_str)
}

pub(super) fn outcome_accepted_or_noop(outcome: &Value) -> bool {
    matches!(outcome_status(outcome), Some("accepted") | Some("noop"))
}

/// JS `acceptedOrNoopCanonicalTaskIdsFrom`.
pub(super) fn accepted_or_noop_canonical_task_ids_from(
    contract: &LifecycleContract<'_>,
    outcomes: &[Value],
) -> Vec<String> {
    let mut ids = Vec::new();
...
pub(super) fn non_accepted_outcomes(outcomes: &[Value]) -> Vec<Value> {
    outcomes
        .iter()
...
pub(super) fn matching_accepted_ids(
    contract: &LifecycleContract<'_>,
    source_items: &[Value],
    outcomes: &[Value],
) -> Vec<String> {
...
pub(super) fn outcomes_of(result: &Value) -> Vec<Value> {
...
pub(super) fn work_type_for(item: &Value) -> &str {
    item.get("work_type").and_then(Value::as_str).unwrap_or("")
}

/// JS `validImplementationItem` / `validVerifiedNoopItem` / `validInventoryItem`.
pub(super) fn valid_inventory_item(contract: &LifecycleContract<'_>, item: &Value) -> bool {
...
/// JS `itemIsCompleted`.
pub(super) fn item_is_completed(
    contract: &LifecycleContract<'_>,
    item: &Value,
    completed: &BTreeSet<String>,
) -> bool {
...
pub(super) fn matching_accepted_noop_ids(
    contract: &LifecycleContract<'_>,
    source_items: &[Value],
    outcomes: &[Value],
) -> Vec<String> {
...
pub(super) fn matching_accepted_completion_ids(
    contract: &LifecycleContract<'_>,
    source_items: &[Value],
    outcomes: &[Value],
) -> Vec<String> {
