...
pub(crate) fn response(prompt:&str)->Option<String>{
 let raw=prompt.split("## Input\n```json\n").nth(1)?.split("\n```").next()?;
 let input:Value=serde_json::from_str(raw).ok()?;
 let contract=input.get("audit_contract")?;
 let root=prompt.lines().find_map(|s|s.strip_prefix("repository_root: "))?;
...
pub(crate) fn outcome(request:&archon_workflow::WorkflowAgentCall)->Option<archon_workflow::WorkflowAgentOutcome>{
 let prompt=request.messages.iter().filter_map(|m|m["content"].as_str()).collect::<Vec<_>>().join("\n");
 Some(archon_workflow::WorkflowAgentOutcome{content:response(&prompt)?,tool_uses:vec![],tokens_in:0,tokens_out:0,stop_reason:Some("end_turn".into())})
}
