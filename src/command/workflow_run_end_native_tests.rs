...
async fn enabled_native_observer_executes_pinned_command_in_scratch() {
    let mut fixture = frozen_fixture(vec![criterion(
        "AC-X-001",
        command("test -f input && printf checked > result".into()),
    )]);
...
async fn native_nested_verifier_cannot_pass_when_its_floor_is_missing() {
    ...
    git(&["commit", "-qm", "source"]);
    let scratch = tempfile::tempdir().unwrap();
    fixture.snapshot.native_execution = Some(
        serde_json::json!({"policy":{"repository":repo.path(),"project":fixture.project.path(),"task_root":fixture.task_root,"scratch_parent":scratch.path(),"project_inputs":[],"combined":true,"toolchain_path":"/usr/bin:/bin","environment":{},"cargo_seed":null,"timeout_secs":10,"output_bytes":2048,"scratch_bytes":16777216},"source_commit":git(&["rev-parse","HEAD"])}),
    );
    let run = fixture.store.create_run(finalizer_spec()).unwrap();
    persist_native_terminal(&fixture, &run.id);
    let outcome = FixedRunEndAcceptanceObserver::new(fixture.store.clone())
        .observe_async(&context(&fixture, &run.id))
        .await
        .unwrap();
    assert!(
        outcome.policy_finding_count > 0,
        "command passing cannot replace its declarative prerequisites"
    );
}
...
async fn observation_parent_entry() {
    ...
    crate::command::acceptance_scratch_guardian::launch(request)
        .await
        .unwrap();
}
...
fn killed_observer_parent_leaves_no_managed_group_or_worktree() {
    ...
    git(&["commit", "-qm", "source"]);
    let scratch = tempfile::tempdir().unwrap();
    let evidence = signals.path().join("evidence");
    ...
    let request = crate::command::acceptance_scratch_guardian::Request {
        policy: archon_workflow::acceptance_scratch::ScratchPolicy {
            repository: repo.path().into(),
            project: fixture.project.path().into(),
    ...
    let request_path = signals.path().join("request.json");
    ...
    while !ready.exists() && Instant::now() < deadline {
        assert!(
            ...
            "parent failed before command readiness"
        );
    ...
    assert_eq!(
        ...
        -1,
        "managed process group survived"
    );
    assert_eq!(
...
async fn native_policy_on_commandless_contract_keeps_existing_floor_evaluation() {
    ...
    fixture.snapshot.native_execution =
        Some(serde_json::json!({"capture_error":"unused native configuration"}));
    let run = fixture.store.create_run(finalizer_spec()).unwrap();
    let outcome = FixedRunEndAcceptanceObserver::new(fixture.store.clone())
        .observe_async(&context(&fixture, &run.id))
        .await
...
async fn native_dispatch_refuses_before_terminal_persistence() {
    ...
    fixture.snapshot.native_execution =
        Some(serde_json::json!({"capture_error":"must not reach policy parsing"}));
    let run = fixture.store.create_run(finalizer_spec()).unwrap();
    let error = FixedRunEndAcceptanceObserver::new(fixture.store.clone())
        .observe_async(&context(&fixture, &run.id))
        .await
        .unwrap_err();
...
fn persist_native_terminal(fixture: &FrozenFixture, run_id: &str) {
    let mut run = fixture.store.load_state(run_id).unwrap();
    ...
    fixture
        .store
        .write_run_json(run_id, FINALIZATION_RECORD_PATH, &record)
        .unwrap();
}
...
#[tokio::test]
async fn native_nested_verifier_executes_after_passing_prerequisites() {
    let mut fixture = frozen_fixture(vec![criterion(
        "AC-X-001",
...
#[tokio::test]
async fn voided_native_observation_retains_run_evidence() {
    let mut fixture = frozen_fixture(vec![criterion(
        "AC-X-001",
        command("test -f input && rm ../repo/.git".into()),
    )]);
    let repo = tempfile::tempdir().unwrap();
    let git = |args: &[&str]| {
        let o = std::process::Command::new("git")
