...
async fn a_failed_script_keeps_its_own_status() {
    ...
    let (gated, gate) = apply_acceptance_gate(
        &run.store,
        &run.run_id,
        &run.v2_store,
...
// The authored run's terminal rule over host records; split out to hold the
// 500-line ceiling.
#[path = "workflow_live_v3_run_outcome_tests.rs"]
mod outcome;
