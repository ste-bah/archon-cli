...
//! the package level; `default-features = false` does not help, because the
//! edge is direct rather than through `archon-core`.
//!
//! The split is not only about a dependency. Every consumer of this file --
//! `topology_lint`, `sona_workflow_shape_gate`, and the PRD layout tests -- is
//! host code that lints or scores a task set. None of them start a workflow,
//! and workflow execution never calls anything here. The parsing this reuses
//! stays where a run needs it.
