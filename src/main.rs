...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        // ── Agent discovery commands (TASK-AGS-306, TASK-AGS-307) ────
        Some(Commands::AgentList { include_invalid }) => {
            use archon_core::agents::catalog::{AgentFilter, DiscoveryCatalog};
            ...
            let agents_dir = working_dir_for_config.join(".archon/agents");
        ...
        Some(Commands::AgentSearch {
            ...
            let filter = AgentFilter {
                ...
                logic: if logic.eq_ignore_ascii_case("or") { FilterLogic::Or } else { FilterLogic::And },
                include_invalid,
            };
            let results = catalog.list(&filter);
            println!("{:<30} {:<10} {:<15} {}", "NAME", "VERSION", "CATEGORY", "DESCRIPTION");
        ...
        Some(Commands::AgentInfo { name, version, json }) => {
            ...
            let info = match catalog.info(&name, version_req.as_ref()) {
                Ok(info) => info,
                Err(archon_core::agents::catalog::DiscoveryError::AgentNotFound { name, suggestions }) => {
                    eprintln!("Agent '{name}' not found. Did you mean: {}?", suggestions.join(", "));
                    std::process::exit(12);
                }
                Err(e) => {
                    eprintln!("Error: {e}");
            ...
            if json {
                ...
                println!("Resources:    cpu={} memory={}MB timeout={}s",
                ...
                println!("Input schema:  {}", serde_json::to_string(&info.selected.input_schema).unwrap_or_default());
                println!("Output schema: {}", serde_json::to_string(&info.selected.output_schema).unwrap_or_default());
            }
            return Ok(());
        }
        None => {}
