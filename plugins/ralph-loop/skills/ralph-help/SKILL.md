---
name: ralph-help
description: Explain the Ralph Loop plugin, its skills, and the Ralph Wiggum technique; use when the user asks how ralph-loop works or for help with it.
...
**How it works:**
1. Creates `.archon/ralph-loop.local.md` state file
...
- Removes `.archon/ralph-loop.local.md`
...
On Archon versions whose Stop hook event provides no transcript, the tag cannot be detected automatically; the loop's continuation message then instructs the agent to set `active: false` in the state file when (and only when) the promise statement is genuinely true.
