...
Pure data. No env-var reads, no I/O, no debug_log — kept side-effect-free so
it can be imported in isolation.

Note: the upstream Claude Code plugin also defined a RuleId enum and bitmask
helper used only by its telemetry channel; Archon has no equivalent metrics
channel, so those are not ported.
"""
...
Only use exec() if you absolutely need shell features and the input is guaranteed to be safe.""",
    ...
    },
    {
        ...
        # innerHTML strings with zero user input.
        "path_filter": lambda p: p.endswith(_JS_EXTS),
...
- For numeric values: parse to int/float first""",
    ...
    },
    {
        ...
        # Bare yaml.load() is unsafe_yaml_load's job.
