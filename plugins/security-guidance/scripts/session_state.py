"""
Per-session state-file plumbing for the security-guidance plugin (Archon port).

Holds the JSON state file location, fcntl-locked read-modify-write helper,
and old-file GC. Side-effect-free at import time.
"""
try:
    import fcntl
except ImportError:
...
def _state_key(session_id):
    # The key becomes a filename component under the state dir. Archon session
    # ids are typically UUID-like (sanitization is a no-op for them), but
    # nothing in the hook protocol guarantees that, so strip path separators
    # and anything else that could escape the state dir, and bound the length.
