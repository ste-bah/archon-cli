...
fn test_pipeline_code_parses() {
    let cli = Cli::try_parse_from(["archon", "pipeline", "code", "implement hello world"]).unwrap();
    match cli.command {
        Some(Commands::Pipeline {
            action: PipelineAction::Code { task, dry_run },
        }) => {
...
fn test_pipeline_code_dry_run_parses() {
    ...
    match cli.command {
        Some(Commands::Pipeline {
            action: PipelineAction::Code { dry_run, .. },
        }) => {
            assert!(dry_run);
        }
...
fn test_pipeline_research_parses() {
    let cli = Cli::try_parse_from(["archon", "pipeline", "research", "quantum computing"]).unwrap();
    match cli.command {
        Some(Commands::Pipeline {
            action: PipelineAction::Research { topic, dry_run },
        }) => {
...
fn test_pipeline_status_parses() {
    ...
    match cli.command {
        Some(Commands::Pipeline {
            action: PipelineAction::Status { session_id },
        }) => {
...
fn test_pipeline_abort_parses() {
    ...
    match cli.command {
        Some(Commands::Pipeline {
            action: PipelineAction::Abort { session_id },
        }) => {
            assert_eq!(session_id, "abc-123");
        }
