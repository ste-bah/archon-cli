...
- v1.6.0 lets agents hand work to each other: a durable task board with atomic claims that survive the memory garden, subagent identity so a write can be attributed, a self-check that reports a file over the line limit back into the agent own transcript, and a browser view of what is running. Also fixes consolidation silently reporting an unavailable semantic pass as a clean store on any second archon instance.
