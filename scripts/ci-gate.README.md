...
| 2c| deferral-markers| `TODO(TICKET)` only where allowlisted        | issue #189   |
| 2d| gate-self-tests | The guards above can still go red            | issue #189   |
...
- `--only <step>` — run a single step and exit. Step keys are:
  `file-sizes`, `banned-imports`, `r0-entry-gate`, `deferral-markers`,
  `gate-self-tests`, `fmt`, `clippy`, `test`, `baseline-diff`, `bench`.
