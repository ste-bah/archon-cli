#!/usr/bin/env bash
# Self-test for check-coverage.sh (TASK-TUI-809).
#
...
# instead of running `check-coverage.sh` itself, because the real script
# targets `-p archon-tui` in the parent workspace (running the full archon-tui
# test suite under llvm-cov can crash WSL2). The flag surface under test
# (--fail-under-lines + --summary-only) is identical, and this is the only
# way to get a hermetic, focused run. Mirrors the TUI-808 pattern.
set -euo pipefail
...
if [[ ! -x "$CHECKER" ]]; then
    echo "FAIL: check-coverage.sh not executable at $CHECKER"
    exit 1
fi
...
WORK="$(mktemp -d -t covergate-self-test-XXXXXX)"
